...
//delete all activity
    ...
    if($d){
        ...
        _log($msg_type.' Deleted: '.$d['name'].' [ID: '.$d['id'].']','Admin',$user['id']);

        $d->delete();

        r2(U.$r,'s', $msg_type. ' ' .$_L['Deleted Successfully']);
    ...
    case 'invoice':
        ...
        if($d){
            //delete all invoice transactions
            $x = ORM::for_table('sys_transactions')->where('iid',$id)->delete_many();
            $x = ORM::for_table('sys_transactions')->where('category','Invoice')->where('ref',$id)->delete_many();
