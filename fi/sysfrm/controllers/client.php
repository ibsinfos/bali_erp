...
require('sysfrm/lib/razorpay/Razorpay.php');
    ...
    case 'ipdf':
        ...
        if($d){
          ...
          //  $i_due = number_format($i_due,2,$config['dec_point'],$config['thousands_sep']);
            ...
            $pdf_tpl = 'sysfrm/lib/invoices/pdf-x2.php';

            Event::trigger('rendering_pdf_invoice',array($id));
    ...
    case 'ipay':
        ...
        if($d){
            ...
            switch ($pg){
                ...
                case 'razorpay':
                    $processor_details = ORM::for_table('sys_pg')->where('processor',$pg)->find_one();
                    $gatewayMixDataArr= explode(',',$processor_details['value']);
                    
                    $keyId=$gatewayMixDataArr[0];
                    $keySecret=$gatewayMixDataArr[1];
                    //echo '<pre>';print_r($gatewayMixDataArr);die;
                    ...
                    $ui->assign('name', $a['fname']." ".$a['lname']);
                    $ui->assign('description', "School fees payment");
                    $ui->assign('image', "http://sharadtechnologies.com/wp-content/uploads/2016/07/logo.png");
                    $ui->assign('email', $a['email']);
                    $ui->assign('contact', $a['phone']);
                    $ui->assign('order_id', $invoiceid);
                    $ui->assign('razorpayOrderId', $razorpayOrderId);
                    $ui->assign('id', $id);
                    $ui->assign('token', $token);
                    
                    $json = json_encode($data);
                    //die("hear i am");
                    //$ui->assign('xfooter', $xfooter);
                    $ui->assign('displayCurrency', 'INR');
                    //$ui->assign('data', $data);
                    $ui->display('razorpay.tpl');
...
//                        $ui->display('client-ipay.tpl');
    ...
    case 'login':
        ...
        if (isset($_SERVER['SERVER_NAME'])) {
            ...
            $url=$server.$url_arr[1]."/index.php?parents/dashboard";
            header('location: '.$url);
...
//        $password = _post('password');
        ...
        $auth = Contacts::client_login($email,$password);
        if($auth){
        ...
        if($id!="" && $token != ""){
            ...
            if($razorpay_payment_id != "" && $razorpay_signature!=""){
                try{
                    ...
                    $processor_details = ORM::for_table('sys_pg')->where('processor','razorpay')->find_one();
                    $gatewayMixDataArr= explode(',',$processor_details['value']);
                    ...
                    //$id  = $routes['2'];
                    //$token = $routes['3'];
                    r2(U."client/iview/$id/$token/",'e',$error);
                }
                ...
                if($d){
                    ...
                    if($token != $vtoken){
                        ...
                        if ($i) {
                            ...
                            $accountname=$processor_details['name'];
                            $i->credit = $pc + $amount;
                            $i->paymentmethod = $accountname;
                            $i->save();
                            ...
                            $ins="Unknow error arises about the transaction,Please contact to school.";
                            r2(U.'client/iview/'.$i->id.'/'.$i->vtoken.'/','s',$ins);
                        }
                    }
                }
