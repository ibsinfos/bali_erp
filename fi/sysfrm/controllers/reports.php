...
$(\'#dp1\').datepicker({
 ...
 ');
	...
	$d->where('school_id', $_SESSION['school_id']);
        ...
        $d->where_not_equal('category','invoice');
        //$d->order_by_desc('date');
        $d->order_by_desc('id');
        $x =  $d->find_many();
    ...
    case 'by-date':
        ...
        $d = ORM::for_table('sys_transactions')->select('sys_transactions.*')->select('crm_accounts.account','fname')->select('crm_accounts.lname')->where('sys_transactions.date',$mdate)->where('sys_transactions.type','Income')->
    _add_join_source('left', 'crm_accounts',array('sys_transactions.payerid','=','crm_accounts.id'))->where('school_id', $_SESSION['school_id'])->order_by_desc('sys_transactions.id')->find_many();
        
        $dr = ORM::for_table('sys_transactions')->where('school_id', $_SESSION['school_id'])->where('date',$mdate)->sum('dr');
        if($dr == ''){
            $dr = '0.00';
        }
        $cr = ORM::for_table('sys_transactions')->where('school_id', $_SESSION['school_id'])->where('date',$mdate)->sum('cr');
        if($cr == ''){
            $cr = '0.00';
        }
        $ui->assign('d',$d);
        ...
        <link rel="stylesheet" type="text/css" href="' . $_theme . '../../../lib/datatable/css/jquery.dataTables.min.css"/>
        ');
        $ui->assign('xfooter', '
        ...
        <script type="text/javascript" src="' . $_theme . '../../../lib/datatable/js/jquery.dataTables.min.js"></script>');
 ...
 ');
    ...
    case 'income':
        ...
        $d = ORM::for_table('sys_transactions')->select('sys_transactions.*')->select('sys_accounts.account')
                                      ->select('sys_invoices.account','payer_name')
                                      ->_add_join_source( 'left' , 'sys_accounts' , array( 'sys_transactions.account' , '=' , 'sys_accounts.id' ))
                                      ->_add_join_source('left','sys_invoices', array('sys_invoices.id', '=', 'sys_transactions.iid'))
                                      ->where('type','Income')->where('school_id',$_SESSION['school_id'])->limit(20)->order_by_desc('id')->find_many();
        
        //echo ORM::get_last_query();exit;
//      $d = ORM::for_table('sys_transactions')->where('type','Expense')->limit(20)->order_by_desc('id')->find_many();
        $ui->assign('d',$d);
        $a = ORM::for_table('sys_transactions')->where('school_id',$_SESSION['school_id'])->sum('cr');
        if($a == ''){
            $a = '0.00';
        }
        $ui->assign('a',$a);
        $m = ORM::for_table('sys_transactions')->where('school_id',$_SESSION['school_id'])->where_gte('date',$first_day_month)->where_lte('date',$mdate)->sum('cr');
        if($m == ''){
            $m = '0.00';
        }
        $ui->assign('m',$m);

        $w = ORM::for_table('sys_transactions')->where('school_id',$_SESSION['school_id'])->where_gte('date',$this_week_start)->where_lte('date',$mdate)->sum('cr');
        if($w == ''){
            $w = '0.00';
        }
        $ui->assign('w',$w);

        $m3 = ORM::for_table('sys_transactions')->where('school_id',$_SESSION['school_id'])->where_gte('date',$before_30_days)->where_lte('date',$mdate)->sum('cr');
        if($m3 == ''){
            $m3 = '0.00';
        }
        $ui->assign('m3',$m3);
...
//generate graph string
        ...
        for ($m=0; $m<=$till; $m++) {
            $mnth = $array[$m];
            $cal = ORM::for_table('sys_transactions')->where('school_id',$_SESSION['school_id'])->where_gte('date',date('Y-m-d',strtotime("first day of $mnth")))->where_lte('date',date('Y-m-d',strtotime("last day of $mnth")))->sum('cr');
 ...
 ');
    ...
    case 'expense':
        ...
        $d = ORM::for_table('sys_transactions')->select('sys_transactions.*')->select('sys_accounts.account')
                            ->select('sys_invoices.account','payee_name')
                            ->_add_join_source( 'left' , 'sys_accounts' , array( 'sys_transactions.account' , '=' , 'sys_accounts.id' ))
                            ->_add_join_source('left','sys_invoices', array('sys_invoices.id', '=', 'sys_transactions.iid'))
                            ->where('type','Expense')->where('school_id',$_SESSION['school_id'])->limit(20)->order_by_desc('id')->find_many();
//echo ORM::get_last_query();exit;
        $ui->assign('d',$d);
        $a = ORM::for_table('sys_transactions')->where('school_id',$_SESSION['school_id'])->sum('dr');
        if($a == ''){
            $a = '0.00';
        }
        $ui->assign('a',$a);
        $m = ORM::for_table('sys_transactions')->where('school_id',$_SESSION['school_id'])->where_gte('date',$first_day_month)->where_lte('date',$mdate)->sum('dr');
        if($m == ''){
            $m = '0.00';
        }
        $ui->assign('m',$m);

        $w = ORM::for_table('sys_transactions')->where('school_id',$_SESSION['school_id'])->where_gte('date',$this_week_start)->where_lte('date',$mdate)->sum('dr');
        if($w == ''){
            $w = '0.00';
        }
        $ui->assign('w',$w);

        $m3 = ORM::for_table('sys_transactions')->where('school_id',$_SESSION['school_id'])->where_gte('date',$before_30_days)->where_lte('date',$mdate)->sum('dr');
        if($m3 == ''){
            $m3 = '0.00';
        }
        $ui->assign('m3',$m3);
...
//generate graph string
        ...
        for ($m=0; $m<=$till; $m++) {
            $mnth = $array[$m];
            $cal = ORM::for_table('sys_transactions')->where('school_id',$_SESSION['school_id'])->where_gte('date',date('Y-m-d',strtotime("first day of $mnth")))->where_lte('date',date('Y-m-d',strtotime("last day of $mnth")))->sum('dr');
            $gstring .= '["'.ib_lan_get_line($mnth).'",'.$cal.'], ';
 ...
 ');
    ...
    case 'income-vs-expense':
        ...
        $ai = ORM::for_table('sys_transactions')->where('school_id',$_SESSION['school_id'])->sum('cr');
        if($ai == ''){
            $ai = '0.00';
        }
        $ui->assign('ai',$ai);
        $mi = ORM::for_table('sys_transactions')->where('school_id',$_SESSION['school_id'])->where_gte('date',$first_day_month)->where_lte('date',$mdate)->sum('cr');
        if($mi == ''){
            $mi = '0.00';
        }
        $ui->assign('mi',$mi);

        $wi = ORM::for_table('sys_transactions')->where('school_id',$_SESSION['school_id'])->where_gte('date',$this_week_start)->where_lte('date',$mdate)->sum('cr');
        if($wi == ''){
            $wi = '0.00';
        }
        $ui->assign('wi',$wi);

        $m3i = ORM::for_table('sys_transactions')->where('school_id',$_SESSION['school_id'])->where_gte('date',$before_30_days)->where_lte('date',$mdate)->sum('cr');
        if($m3i == ''){
            $m3i = '0.00';
        }
        $ui->assign('m3i',$m3i);

        $ae = ORM::for_table('sys_transactions')->where('school_id',$_SESSION['school_id'])->sum('dr');
        if($ae == ''){
            $ae = '0.00';
        }
        $ui->assign('ae',$ae);
        $me = ORM::for_table('sys_transactions')->where('school_id',$_SESSION['school_id'])->where_gte('date',$first_day_month)->where_lte('date',$mdate)->sum('dr');
        if($me == ''){
            $me = '0.00';
        }
        $ui->assign('me',$me);
...
');
 ...
 ');
    ...
    case 'categories':
        ...
        $d = ORM::for_table('sys_cats')->where('school_id',$_SESSION['school_id'])->find_many();
        $ui->assign('d', $d);
...
$(\'#dp1\').datepicker({
 ...
 ');
    ...
    case 'payees':
        ...
        $d = ORM::for_table('sys_payee')->where('school_id',$_SESSION['school_id'])->find_many();
        $ui->assign('d', $d);
...
$(\'#dp1\').datepicker({
 ...
 ');
    ...
    case 'payers':
        ...
        $d = ORM::for_table('sys_payers')->where('school_id',$_SESSION['school_id'])->find_many();
        $ui->assign('d', $d);
