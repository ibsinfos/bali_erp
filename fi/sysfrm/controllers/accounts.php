...
//Find all accounts
 ...
 ');
    ...
    case 'add-post':
        ...
        if( $contact_phone != '' && !ctype_digit( $contact_phone )) {
            $msg .= '<i class="fa-fw fa fa-times"></i>Invalid Phone Number '. '<br>';
        }
//check with same name account is exist
        ...
        if($account_number == '') {
            ...
            if(Validator::Length($account_number,15,5) == false){
                $msg .= '<i class="fa-fw fa fa-times"></i>Account Number Should be Valid'. '<br>';
            }
        }
        // From version 4
...
');
    ...
    case 'delete':
        ...
        $d = ORM::for_table('sys_accounts')->find_one($id);
        if($d->balance >0 ){
            r2(U . 'accounts/list', 'e','Cannot process deletion as this account has balance in it.');
