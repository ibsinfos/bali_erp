...
');
    ...
    case 'expense-categories-post':
        ...
        $c = ORM::for_table('sys_cats')->where('school_id',$_SESSION['school_id'])->where('name',$name)->where('type','Expense')->find_one();
        if($c){
    ...
    case 'income-categories':
        ...
        $d = ORM::for_table('sys_cats')->where('type','Income')->where('school_id',$_SESSION['school_id'])->order_by_asc('sorder')->find_many();
        $ui->assign('d',$d);
        $ui->assign('xheader', '
<link rel="stylesheet" type="text/css" href="' . $_theme . '/css/liststyle.css"/>
');
    ...
    case 'income-categories-post':
        ...
        $c = ORM::for_table('sys_cats')->where('name',$name)->where('school_id',$_SESSION['school_id'])->where('type','Income')->find_one();
        if($c){
    ...
    case 'categories-manage':
        ...
        $id = $routes[2];
        $d = ORM::for_table('sys_cats')->where('school_id',$_SESSION['school_id'])->find_one($id);
        if($d){
            $ui->assign('c',$d);
    ...
    case 'categories-edit-post':
        ...
        if($d){
            ...
            $rd = strtolower($otype);
            $name = _post('name');
            $c = ORM::for_table('sys_cats')->where('school_id',$_SESSION['school_id'])->where('name',$name)->where('type',$otype)->find_one();
            if($c){
            ...
            if($name == ''){
                ...
                ORM::for_table('sys_transactions')->raw_execute("update sys_transactions set category='$name' where (category='$oname' AND type='$type' AND school_id = '".$_SESSION['school_id']."')");
    ...
    case 'categories-delete':
        ...
        if($d){
            ...
            ORM::for_table('sys_transactions')->raw_query("update sys_transactions set category=:cat where category='$name' AND type='$type' AND school_id = '".$_SESSION['school_id']."'", array('cat' => 'Uncategorized'));
            $d->delete();
            if($type == 'Income'){
    ...
    case 'payee':
        ...
        $d = ORM::for_table('sys_payee')->where('school_id',$_SESSION['school_id'])->order_by_asc('sorder')->find_many();
        $ui->assign('d',$d);
        $ui->assign('xheader', '
<link rel="stylesheet" type="text/css" href="' . $_theme . '/css/liststyle.css"/>
');
...
');
    ...
    case 'payee-manage':
        ...
        $id = $routes[2];
        $d = ORM::for_table('sys_payee')->where('school_id',$_SESSION['school_id'])->find_one($id);
        if($d){
            $ui->assign('c',$d);
    ...
    case 'payee-edit-post':
        ...
        if($d){
            $name = _post('name');
            $c = ORM::for_table('sys_payee')->where('school_id',$_SESSION['school_id'])->where('name',$name)->find_one();
            if($c){
            ...
            if($name == ''){
                ...
                $d->save();
                //update payee in transactions
                ORM::for_table('sys_transactions')->raw_query("update sys_transactions set payee=:payee where payee='$oname' AND school_id = '".$_SESSION['school_id']."'", array('payee' => $name));
    ...
    case 'payee-post':
        ...
        $c = ORM::for_table('sys_payee')->where('name',$name)->where('school_id',$_SESSION['school_id'])->find_one();
        if($c){
            r2(U."settings/payee",'e',$_L['name_exist_error']);
        }
    ...
    case 'payer':
        ...
        $d = ORM::for_table('sys_payers')->where('school_id',$_SESSION['school_id'])->order_by_asc('sorder')->find_many();
        $ui->assign('d',$d);
        $ui->assign('xheader', '
<link rel="stylesheet" type="text/css" href="' . $_theme . '/css/liststyle.css"/>
');
...
');
    ...
    case 'payer-manage':
        ...
        $id = $routes[2];
        $d = ORM::for_table('sys_payers')->where('school_id',$_SESSION['school_id'])->find_one($id);
        if($d){
            $ui->assign('c',$d);
    ...
    case 'payer-edit-post':
        ...
        if($d){
            $name = _post('name');
            $c = ORM::for_table('sys_payers')->where('school_id',$_SESSION['school_id'])->where('name',$name)->find_one();
            if($c){
            ...
            if($name == ''){
                ...
                ORM::for_table('sys_transactions')->raw_query("update sys_transactions set payer=:payer where payer='$oname' AND school_id = '".$_SESSION['school_id']."'", array('payer' => $name));
    ...
    case 'payer-post':
        ...
        $c = ORM::for_table('sys_payers')->where('name',$name)->where('school_id',$_SESSION['school_id'])->find_one();
        if($c){
            r2(U."settings/payer",'e',$_L['name_exist_error']);
        }
        ...
        $d->name = $name;
        $d->school_name = $_SESSION['school_id'];
        $d->save();
    ...
    case 'pmethods':
        ...
        $d = ORM::for_table('sys_pmethods')->where('school_id',$_SESSION['school_id'])->order_by_asc('sorder')->find_many();
        $ui->assign('d',$d);
        $ui->assign('xheader', '
<link rel="stylesheet" type="text/css" href="' . $_theme . '/css/liststyle.css"/>
');
...
//');
    ...
    case 'pmethods-manage':
        ...
        $id = $routes[2];
        $d = ORM::for_table('sys_pmethods')->where('school_id',$_SESSION['school_id'])->find_one($id);
        if($d){
            $ui->assign('c',$d);
    ...
    case 'pmethods-edit-post':
        ...
        if($d){
            $name = _post('name');
            $c = ORM::for_table('sys_pmethods')->where('school_id',$_SESSION['school_id'])->where('name',$name)->find_one();
            if($c){
            ...
            if($name == ''){
                ...
                ORM::for_table('sys_transactions')->raw_query("update sys_transactions set pmethod=:pmethod where pmethod='$oname' AND school_id = '".$_SESSION['school_id']."'", array('pmethod' => $name));
    ...
    case 'app':
        ...
        $e = ORM::for_table('sys_emailconfig')->where('school_id',$_SESSION['school_id'])->find_one('1');
        $ui->assign('e',$e);
...
//');
        $d = ORM::for_table('sys_users')->where('school_id',$_SESSION['school_id'])->find_many();
        $ui->assign('d',$d);
    ...
    case 'users-edit':
        ...
        $id  = $routes['2'];
        $d = ORM::for_table('sys_users')->where('school_id',$_SESSION['school_id'])->find_one($id);
        if($d){
...
//check with same name account is exist
        $d = ORM::for_table('sys_users')->where('school_id',$_SESSION['school_id'])->where('username',$username)->find_one();
        if($d){
...
//check with same name account is exist
        if($d['username'] != $username){
            $c = ORM::for_table('sys_users')->where('school_id',$_SESSION['school_id'])->where('username',$username)->find_one();
            if($c){
                $msg .= $_L['account_already_exist']. '<br>';
            }
        }
    ...
    case 'app-post':
        ...
        if($_app_stage == 'Demo'){
            ...
            $d = ORM::for_table('sys_appconfig')->where('setting','CompanyName')->where('school_id',$_SESSION['school_id'])->find_one();
            $d->value = $company;
			$d->school_id = $_SESSION['school_id'];									   
            $d->save();

            $d = ORM::for_table('sys_appconfig')->where('setting','theme')->where('school_id',$_SESSION['school_id'])->find_one();
            $d->value = $theme;
			$d->school_id = $_SESSION['school_id'];									   
            $d->save();

            $d = ORM::for_table('sys_appconfig')->where('setting','nstyle')->where('school_id',$_SESSION['school_id'])->find_one();
            $d->value = $nstyle;
			$d->school_id = $_SESSION['school_id'];									   
            $d->save();


            $d = ORM::for_table('sys_appconfig')->where('setting','pdf_font')->where('school_id',$_SESSION['school_id'])->find_one();
            $d->value = $pdf_font;
			$d->school_id = $_SESSION['school_id'];									   
            $d->save();

            $caddress = $_POST['caddress'];
            $d = ORM::for_table('sys_appconfig')->where('setting','caddress')->where('school_id',$_SESSION['school_id'])->find_one();
            $d->value = $caddress;
			$d->school_id = $_SESSION['school_id'];									   
            $d->save();

            $invoice_terms = $_POST['invoice_terms'];
            $d = ORM::for_table('sys_appconfig')->where('setting','invoice_terms')->where('school_id',$_SESSION['school_id'])->find_one();
            ...
            $d = ORM::for_table('sys_appconfig')->where('setting','i_driver')->where('school_id',$_SESSION['school_id'])->find_one();
            $d->value = $i_driver;
			$d->school_id = $_SESSION['school_id'];									   
            $d->save();
            ...
            $d = ORM::for_table('sys_appconfig')->where('setting','default_landing_page')->where('school_id',$_SESSION['school_id'])->find_one();
            ...
            $d = ORM::for_table('sys_appconfig')->where('setting','dashboard')->where('school_id',$_SESSION['school_id'])->find_one();
            $d->value = $dashboard;
			$d->school_id = $_SESSION['school_id'];									   
            $d->save();
    ...
    case 'eml-post':
        ...
        $d = ORM::for_table('sys_appconfig')->where('setting','sysEmail')->where('school_id',$_SESSION['school_id'])->find_one();
        $d->value = $sysemail;
		$d->school_id = $_SESSION['school_id'];									   
        $d->save();
...
//        }
        ...
        $tzone = _post('tzone');
        $d = ORM::for_table('sys_appconfig')->where('setting','timezone')->where('school_id',$_SESSION['school_id'])->find_one();
        $d->value = $tzone;
		$d->school_id = $_SESSION['school_id'];									   
        $d->save();

        $country = _post('country');
        $d = ORM::for_table('sys_appconfig')->where('setting','country')->where('school_id',$_SESSION['school_id'])->find_one();
        $d->value = $country;
		$d->school_id = $_SESSION['school_id'];									   
        $d->save();
...
//        }
        ...
        if($cformat == '1'){
            $d = ORM::for_table('sys_appconfig')->where('setting','dec_point')->where('school_id',$_SESSION['school_id'])->find_one();
            $d->value = '.';
			$d->school_id = $_SESSION['school_id'];									   
            $d->save();
            $d = ORM::for_table('sys_appconfig')->where('setting','thousands_sep')->where('school_id',$_SESSION['school_id'])->find_one();
            $d->value = '';
			$d->school_id = $_SESSION['school_id'];									   
            $d->save();
        }
        ...
        $d = ORM::for_table('sys_appconfig')->where('setting','currency_code')->where('school_id',$_SESSION['school_id'])->find_one();
...
//        $d->save();
        ...
        $lan = _post('lan');
        $d = ORM::for_table('sys_appconfig')->where('setting','language')->where('school_id',$_SESSION['school_id'])->find_one();
        $d->value = $lan;
		$d->school_id = $_SESSION['school_id'];									   
        $d->save();
        ...
        $d = ORM::for_table('sys_appconfig')->where('setting','home_currency')->where('school_id',$_SESSION['school_id'])->find_one();
        $d->value = $home_currency;
		$d->school_id = $_SESSION['school_id'];									   
        $d->save();

        $currency_decimal_digits = _post('currency_decimal_digits');
        $d = ORM::for_table('sys_appconfig')->where('setting','currency_decimal_digits')->where('school_id',$_SESSION['school_id'])->find_one();
        ...
        $d = ORM::for_table('sys_appconfig')->where('setting','currency_symbol_position')->where('school_id',$_SESSION['school_id'])->find_one();
        $d->value = $currency_symbol_position;
		$d->school_id = $_SESSION['school_id'];									   
        $d->save();

        $thousand_separator_placement = _post('thousand_separator_placement');
        $d = ORM::for_table('sys_appconfig')->where('setting','thousand_separator_placement')->where('school_id',$_SESSION['school_id'])->find_one();
...
//exit;
    ...
    case 'change-password-post':
        ...
        $password = _post('password');
        if($password != ''){
            $d = ORM::for_table('sys_users')->where('username',$user['username'])->where('school_id',$_SESSION['school_id'])->find_one();
            if($d){
                $d_pass = $d['password'];
    ...
    case 'networth_goal':
        ...
        if((is_numeric($goal)) AND $goal != ''){
            $d = ORM::for_table('sys_appconfig')->where('setting','networth_goal')->where('school_id',$_SESSION['school_id'])->find_one();
            $d->value = $goal;
			$d->school_id = $_SESSION['school_id'];									   
            $d->save();
    ...
    case 'email-templates':
        $d = ORM::for_table('sys_email_templates')->where('school_id',$_SESSION['school_id'])->find_many();
        $ui->assign('d',$d);
        $ui->assign('xheader', '
...
');
    ...
    case 'email-templates-view':
        ...
        $sid = $routes['2'];
        $d = ORM::for_table('sys_email_templates')->where('school_id',$_SESSION['school_id'])->find_one($sid);
        if($d){
            $ui->assign('d',$d);
...
<div class="modal-footer">
    ...
    case 'update-email-template':
        $id = _post('id');
        $d = ORM::for_table('sys_email_templates')->where('school_id',$_SESSION['school_id'])->find_one($id);
        if($_app_stage == 'Demo'){
    ...
    case 'tags':
        ...
        $d = ORM::for_table('sys_tags')->where('school_id',$_SESSION['school_id'])->find_many();
        $ui->assign('d',$d);
...
$( "#email_method" ).change(function() {
 ...
 ');
    ...
    case 'automation':
        ...
        $cs = ORM::for_table('sys_schedule')->where('school_id',$_SESSION['school_id'])->find_many();
        foreach($cs as $rcs)
        {
...
// ');
    ...
    case 'pg':
        ...
        $d = ORM::for_table('sys_pg')->where('school_id',$_SESSION['school_id'])->order_by_asc('sorder')->find_many();
        $ui->assign('d',$d);
...
');
 ...
 ');
    ...
    case 'pg-conf':
        $ui->assign('content_inner',inner_contents($config['c_cache']));
        $pg = $routes['2'];
        $d = ORM::for_table('sys_pg')->where('school_id',$_SESSION['school_id'])->find_one($pg);
        if($d){
...
');
    ...
    case 'add-tax-post':
        ...
        $d = ORM::for_table('sys_tax')->where('school_id',$_SESSION['school_id'])->create();
        $d->name = $taxname;
        $d->rate = $taxrate;
        $d->school_id = $_SESSION['school_id'];
        $d->save();
    ...
    case 'edit-tax':
        $ui->assign('content_inner',inner_contents($config['c_cache']));
        $tid = $routes['2'];
        $d = ORM::for_table('sys_tax')->where('school_id',$_SESSION['school_id'])->find_one($tid);
        if($d){
            $ui->assign('xfooter', '
...
');
    ...
    case 'consolekey_regen':
        ...
        $d = ORM::for_table('sys_appconfig')->where('school_id',$_SESSION['school_id'])->where('setting','ckey')->find_one();
        $d->value = $nkey;
		$d->school_id = $_SESSION['school_id'];									   
        $d->save();
    ...
    case 'automation-post':
        ...
        $d = ORM::for_table('sys_schedule')->where('school_id',$_SESSION['school_id'])->where('cname', 'accounting_snapshot')->find_one();
        ...
        $d = ORM::for_table('sys_schedule')->where('school_id',$_SESSION['school_id'])->where('cname', 'recurring_invoice')->find_one();
        ...
        $d = ORM::for_table('sys_schedule')->where('school_id',$_SESSION['school_id'])->where('cname', 'notify')->find_one();
        if ($notify == 'on') {
            $d->val = 'Active';
        } else {
            $d->val = 'Inactive';
        ...
        $d = ORM::for_table('sys_schedule')->where('school_id',$_SESSION['school_id'])->where('cname', 'notifyemail')->find_one();
        $d->val = $notifyemail;
        $d->school_id = $_SESSION['school_id'];
        $d->save();
...
');
        ...
        foreach ($pls as $pl){
            ...
            if(file_exists($pl_path.'/manifest.php')){
                $i++;
                //
                $d = ORM::for_table('sys_pl')->where('school_id',$_SESSION['school_id'])->where('c',$pl)->find_one();
                $btn = '';
                if($d){
...
';
                ...
                $d = ORM::for_table('sys_pl')->where('c',$pl)->where('school_id',$_SESSION['school_id'])->find_one();
                if($d){
                    $d->delete();
...
');
        $cf = ORM::for_table('crm_customfields')->where('ctype','crm')->where('school_id',$_SESSION['school_id'])->order_by_asc('id')->find_many();
    ...
    case 'customfields-ajax-edit':
        ...
        $d = ORM::for_table('crm_customfields')->where('school_id',$_SESSION['school_id'])->find_one($id);
        if($d){
            $ui->assign('d',$d);
...
//    API Support from Version 3
    ...
    case 'api':
        $ui->assign('content_inner',inner_contents($config['c_cache']));
        $d = ORM::for_table('sys_api')->where('school_id',$_SESSION['school_id'])->find_many();
    ...
    case 'api_post':
        ...
        if($label == ''){
           r2(U.'settings/api/','e','Label is Required');
            ...
            $d = ORM::for_table('sys_api')->where('school_id',$_SESSION['school_id'])->create();
            $d->label = $label;
            $d->ip = '';
            $d->apikey = $string;
            $d->school_id = $_SESSION['school_id'];
    ...
    case 'api_delete':
        ...
        $id = $routes[2];
        $d = ORM::for_table('sys_api')->where('school_id',$_SESSION['school_id'])->find_one($id);
        if($d){
...
//        curl_close ($ch);
    ...
    case 'about':
        ...
        $ui->assign('xfooter',Asset::js(array('settings/about')));
...
//        }
    ...
    case 'backup_logo':
        header('Content-Type: application/json');
        $file = 'sysfrm/uploads/system/logo.png';
        $newfile = './logo.png';
...
//        }
    ...
    case 'update_complete':
        ...
        if(file_exists('./logo.png')){
            ...
            rename('./logo.png', 'sysfrm/uploads/system/logo.png');
...
//            $ui->assign('_st', $_L['Settings']);
    ...
    case 'url_rewrite_enable':
        ...
        echo 'URL rewrite enabled... <br> Checking if it\'s working...';
    ...
    case 'school_perfomance':
        $school_perfomance          =   ORM::for_table('sys_perfomancesettings')->where('school_id',$_SESSION['school_id'])->find_many();
        $ui->assign('perfomance',$school_perfomance);
        $ui->assign('count',1);
        $ui->assign('content_inner',inner_contents($config['c_cache']));
        $ui->display('school-perfomance.tpl');
    break;
    
    case 'add_school_perfomance':
        $running_year           =   _post('affected_from');
        $school_perfomance      =   _post('school_perfomance');
        if($school_perfomance == 1) {
            $perfomance_name        =   'Outstanding';
            $perfomance_value       =   2;
        } else if($school_perfomance == 2) {
            $perfomance_name        =   'Very Good';
            $perfomance_value       =   1.75;
        } else if($school_perfomance == 3) {
            $perfomance_name        =   'Good';
            $perfomance_value       =   1.5;
        } else if($school_perfomance == 4) {
            $perfomance_name        =   'Acceptable';
            $perfomance_value       =   1;
        } else if($school_perfomance == 5) {
            $perfomance_name        =   'Weak';
            $perfomance_value       =   1;
        } else {
            $perfomance_name        =   'Very Weak';
            $perfomance_value       =   1;
        }
        
        $d                      =   ORM::for_table('sys_perfomancesettings')->where('school_id',$_SESSION['school_id'])->where( 'running_year' , $running_year )->where( 'active' , '1' )->find_many();
        
        if(count($d)>0) { 
            foreach($d as $key=>$val) {
                $e  =   ORM::for_table('sys_perfomancesettings')->where('school_id',$_SESSION['school_id'])->where('id',$val->id)->find_one();
                $e->active              =   '0';
        ...
        $e                      =   ORM::for_table('sys_perfomancesettings')->create();
        $e->school_perfomance   =   $perfomance_name;
        $e->value               =   $perfomance_value;
        $e->running_year        =   $running_year;
        $e->created_by          =   1;
        $e->school_id           =   $_SESSION['school_id'];
        $e->save();

        r2(U.'settings/school_perfomance','s',$_L['School Perfomance Saved Successfully']);
        
    break;
        
    case 'fee_penalty':
        $fee_penalty          =   ORM::for_table('sys_fee_penalty')->where('school_id',$_SESSION['school_id'])->find_many();
        $ui->assign('penalty',$fee_penalty);
        $ui->assign('count',1);
        $ui->assign('content_inner',inner_contents($config['c_cache']));
        $ui->display('fee_penalty.tpl');
    break;
    case 'add_penalty':
        $academic_year              =   _post('academic_year');
        $penalty_name               =   _post('penalty_name');
        //$amount_per_day             =   _post('amount_per_day');
        $penalty_type             =   _post('penalty_type');
        $amount             =   _post('amount');

        if($penalty_name == ''){
            r2(U."settings/fee_penalty",'e',$_L['Fee_penalty_error']);
        }
        /* if($amount_per_day == ''){
            r2(U."settings/fee_penalty",'e',$_L['Fee_amount_per_day']);
        } */
        if($amount == ''){
            r2(U."settings/fee_penalty",'e',$_L['Amount']);
        }
        
        $d                      =   ORM::for_table('sys_fee_penalty')->where( 'academic_year' , $academic_year )->where( 'status' , '1' )->find_many();
        
        if(count($d)>0) { 
            foreach($d as $key=>$val) {//where('school_id',$_SESSION['school_id'])->
                $e  =   ORM::for_table('sys_fee_penalty')->where('id',$val->id)->find_one();
                $e->status              =   '0';
                $e->school_id           =   $_SESSION['school_id'];
                $e->save();
            }
        }
        $e                      =   ORM::for_table('sys_fee_penalty')->create();
        $e->academic_year       =   $academic_year;
        $e->penalty_name        =   $penalty_name;
        $e->penalty_type      =   $penalty_type;
        $e->amount      =   $amount;
        //$e->amount_per_day      =   $amount_per_day;
        $e->created_by          =   1;
        $e->created_date        =   date('Y-m-d');
        $e->status              =   1;
        $e->school_id           =   $_SESSION['school_id'];
        $e->save();

        r2(U.'settings/fee_penalty','s',$_L['Penalty Fee Saved Successfully']);
    break;
