                ...
                <div class="panel-body">
                    ...
                    <div>
                        <div class="form-group col-md-12">
                            <div class="col-sm-6"><label for="account" class="col-sm-3 control-label">{$_L['Account']}</label></div>
                            <div class="col-sm-6">
                                <select id="account" name="account" class="form-control">
                                    <option value="">{$_L['Choose an Account']}</option>
                                    {foreach $accounts as $ds}
                                        <option {if $trans[0]->account eq $ds['id']} selected="selected" {/if} value="{$ds['id']}">{$ds['account']}</option>
                                    {/foreach}
...
{*                            <input type='radio' id='create_mode' class="create_mode" name="create_mode" value='multiple' > Multiple Invoice*}
                        ...
                        <div class="form-group col-md-6">
                            ...
                            <select id="group_id" name="group_id" class="form-control" disabled="true">
                                    <option value="">{'Select Group'}</option>
                                    {foreach $groups as $group}
                                        <option value="{$group['id']}" {if $a->gid eq $group['id']} selected="selected" {/if} >
                                            {$group['gname']}
                                        </option>
                                    {/foreach}
                            </select>
                        ...
                        <div class="form-group col-md-6 single-invoice-student" >
                            <label for="cid">{'Student'}</label>

                            <select id="cid" name="cid" class="form-control" disabled="true">
                                <option value="">{'Select Student'}...</option>
                                {foreach $c as $cs}
                                    <option value="{$cs['id']}"
                                            {if $trans[0]->payeeid eq ($cs['id'])}selected="selected" {/if}>{$cs['account']} {if $cs['lname'] neq ''}- {$cs['lname']}{/if}</option>
                                {/foreach}
                        ...
                        <table class="table invoice-table" id="invoice_items">
                            ...
                            {foreach $items as $item}
                                <tr> <td>{$item['itemcode']}</td> 
                                    <td>
                                        {$item['description']}
                                        <input type="hidden" class="form-control " name="desc[]" value="{$item['description']}" rows="3"> </td> 
                                    <td style='text-align: left;'>{if ($_c['dec_point']) eq ','}{$item['qty']|replace:'.':','}{else}{$item['qty']}{/if}</td> 
                                    <td style='text-align: left;'>
                                        {if ($_c['dec_point']) eq ','}{$item['amount']|replace:'.':','}{else}{$item['amount']}{/if}
                                        <input type="hidden" class="form-control item_price" name="amount[]" value="{if ($_c['dec_point']) eq ','}{$item['amount']|replace:'.':','}{else}{$item['amount']}{/if}"></td> 
                                    <td style='text-align: left;' class="ltotal">
                                        {if ($_c['dec_point']) eq ','}{{$item['total']}|replace:'.':','}{else}{{$item['total']}}{/if}
                                        <input type="hidden" class="form-control lvtotal" readonly="" value="{if ($_c['dec_point']) eq ','}{{$item['total']}|replace:'.':','}{else}{{$item['total']}}{/if}"></td> 
                                    {*<td> 
                                        <select class="form-control taxed" name="taxed[]"> 
                                            <option value="Yes" {if $item['taxed'] eq '1'}selected=""{/if}>Yes</option> 
                                            <option value="No" {if $item['taxed'] eq '0'}selected=""{/if}>No</option>
                                        </select>
                                    </td>*}
                                </tr>
                            {/foreach}
