...
{*                    <a href="{$_url}invoices/edit/{$d['id']}" class="btn btn-warning  btn-sm"><i class="fa fa-pencil"></i> {$_L['Edit']}</a>*}
                    ...
                    <a href="{$_url}iview/print/{$d['id']}/token_{$d['vtoken']}" target="_blank" class="btn btn-primary  btn-sm"><i class="fa fa-print"></i> {$_L['Print']} {' Invoice'}</a>
                    {if $d['status'] eq 'Paid' || $d['status']  eq 'Partially Paid'}
                    <a href="{$_url}iview/recipt_print/{$d['id']}/token_{$d['vtoken']}" target="_blank" class="btn btn-primary  btn-sm"><i class="fa fa-print"></i> {'Print Receipt'}</a>
                    {/if}
                </div>
                    ...
                    <header class="clearfix">
                        <div class="row">
                            ...
                            <div class="col-sm-6"><span class="pull-right">
                                ...
                                <div class="ib">
                                    <img src="{$app_url}sysfrm/uploads/system/logo.png" width="40" height="40"  alt="Logo">
                                </div></span>
                            </div>
                        </div>
                    </header>
                            ...
                            <div class="col-md-6">
                                <div class="bill-to">
                                    ...
                                    <address>
                                        ...
                                        {$a['address']} <br>
                                        {$a['city']} <br>
                                        {$a['state']} {if $a['zip'] neq ''}-{/if} {$a['zip']}
                                        <br>
                                        <strong>{$_L['Phone']}:</strong> {$a['phone']}
                                        <br>
                                        <strong>{$_L['Email']}:</strong> {$a['parent_email']}
                                        {$x_html}
                                    </address>
                                </div>
                            </div>
                    ...
                    <div class="table-responsive">
                        <table class="table invoice-items">
                            ...
                            <tr class="h4 text-dark">
                                <th id="cell-id" class="text-semibold">#{$d['penalty_id']}</th>
                                <th id="cell-item" class="text-semibold">{'Particulars'}</th>
                                <th id="cell-price" class="text-center text-semibold">{'Amount'}</th>
                            ...
                            {foreach $items as $item}
                                <tr>
                                    ...
                                    <td class="text-center amount" min="-9999999">{$item['amount']}</td>
                            ...
                            {if $d['penalty_id'] neq '' }
                                <tr>
                                    <td>1221{$item['itemcode']+1}</td>
                                    <td class="text-semibold text-dark">{'Fee Penalty'}</td>

                                    <td class="text-center amount">{$d['penalty_amount']}</td>
                            ...
                            <div class="col-sm-4 col-sm-offset-8">
                                <table class="table h5 text-dark">
                                    ...
                                    {if ($d['discount']) neq '0.00'}
                                        <tr>
                                            <td colspan="2">{'Scholarship Discount'}{*$_L['Discount']*}</td>
