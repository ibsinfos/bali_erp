...
class Zend_View_Helper_Grid extends Zend_View_Helper_Abstract {
    ...
    public function grid($dataArray) {
        ...
        //sapp_Payrollcal::pre($dataArray);die;    
        ...
        //sapp_Payrollcal::pre($paginator);die;
        //die("MM");
        ...
        if (isset($dataArray['formgrid']) && $dataArray['formgrid'] == 'true') {
            ...
            if ($dataArray['objectname'] == 'payrollgroup' || $dataArray['objectname'] == 'payrollcategory')
                $view_str = '';
            else if($controllerName == 'payslipgroup' || $controllerName == 'payslipemployee') {                            //echo 'kk'.PHP_EOL;
                //$view_str = '<a href="' . BASE_URL . 'payslipgroup/generate/id/{{id}}" onclick="return showConfirm();" name="{{id}}"  title="Generate">Generate</a>';
                $view_str = '';
            } else {
                //echo 'll'.PHP_EOL;
                $view_str = '<a href= "' . BASE_URL . $dataArray['objectname'] . '/view/id/{{id}}" name="{{id}}" class="sprite view"  title=\'View\'></a>';
            ...
            if ($controllerName == 'payslipgroup' || $controllerName == 'payslipemployee') {
                ...
                $delete_str = '';
            } else if ($dataArray['objectname'] == 'payrollcategory')
                $delete_str = '<a href="' . BASE_URL . 'payrollcategory/delete/id/{{id}}" onclick="return showConfirm();" name="{{id}}" class="sprite delete" title="Delete Category"></a>';
            else if ($dataArray['objectname'] == 'pendingleaves')
            ...
            //echo '<pre>'; print_r($extra['action']);die;
        ...
        if (isset($dataArray['category_id']))
            $this->pd_category_id = $dataArray['category_id'];

        /** capture category name, for policy documents context * */
        if (isset($dataArray['categoryName']))
            $this->pd_category_name = $dataArray['categoryName'];
        //sapp_Payrollcal::pre($dataArray);die;
        ...
        if (!empty($fields)) {
            ...
            foreach ($fields as $key => $value) {
                ...
                if ($value == 'Action')
                    ...
                    $width = ''; //'width='.$eachColumnWidth;
                $output .= "<th " . $width . ">";
                // Check if Sorting is set to True
                if ($sorting) {
                    //sapp_Payrollcal::pre(@$this->extra);die;
                    ...
                    if (@$this->extra[$key]['name'] != '' && !is_array($value)) {
                        ...
                        if ($formgrid == 'true') {
                            ...
                            if($tabindx == '0' && ($name == 'payslipemployee' || $name == 'payslipgroup')) {
                                $output .= "<a href='javascript:void(0);'>".$value."</a>";
                            } else {
                    ...
                    if (@$this->extra[$key]['name'] != '' && !is_array($value)) {
                        ...
                        if ($key != 'id' && $key != 'initialize_status' && $key != 'appraisal_process_status') {
                            ...
                            if ($controllerName == 'myemployees' && $actionName == 'index')
                                $display = '';
                            //sapp_Payrollcal::pre($searchArray);die;
                            ...
                            if (isset($search_filters[$key])) {
                                ...
                                if($controllerName == 'payslipgroup' || $controllerName == 'payslipemployee')
                                    $output .= "<input tabIndex=$tabindx type='text' name='$controllerName' id='$key' style='$display' class='searchtxtbox_$controllerName table_inputs grid_search_inputs' value=\"$sText\" onKeypress='if (!((event.keyCode || event.which) > 47 && (event.keyCode || event.which) < 58) && !((event.keyCode || event.which) > 64  && (event.keyCode || event.which) < 91)  && !((event.keyCode || event.which) > 96  && (event.keyCode || event.which) < 123) && ((event.keyCode || event.which) != 45) && ((event.keyCode || event.which) != 63) && ((event.keyCode || event.which) != 39) && ((event.keyCode || event.which) != 46) && ((event.keyCode || event.which) != 44) && ((event.keyCode || event.which) != 47) && ((event.keyCode || event.which) != 35) && ((event.keyCode || event.which) != 64) && ((event.keyCode || event.which) != 36) && ((event.keyCode || event.which) != 38) && ((event.keyCode || event.which) != 42) && ((event.keyCode || event.which) != 40) && ((event.keyCode || event.which) != 41) && ((event.keyCode || event.which) != 33) && ((event.keyCode || event.which) != 32) && ((event.keyCode || event.which) != 8)) event.preventDefault();' onkeydown='getsearchdata(\"$controllerName\",\"\",this.id,event,\"text\")' />";
                                else{
                                    if($key!='value_formula')
        ...
        foreach ($paginator as $p) {
            ...
            if (!empty($fields)) {
                ...
                foreach ($fields AS $k => $v) {
                    //sapp_Payrollcal::pre($k); 
                    //sapp_Payrollcal::pre($v);
                    $tdclass = '';
                    ...
                    if($k=='generate'){
                        $link=$this->extra['action']['value'];
                        $link=str_replace('{{id}}',$p['id'],$link);
                        //die($link);        
                        $output .= $link;
                        //sapp_Payrollcal::pre($output); //die;
                    }else if (isset($this->extra[$k]['value'])) { //echo $this->extra[$k]['value'];die;
                        $output .= $this->_parseExtra($k, $p);
                    }else if($k=='value_formula'){
                        $p = (array) $p;
                        //sapp_Payrollcal::pre($p);
                        if($p['value_type_id']==2){
                            $get_payroll_condition_value= sapp_Showpayrollcondition::get_payroll_condition_value($p['value_formula']);
                            $get_payroll_condition_value_arr= explode('then', $get_payroll_condition_value);
                            //sapp_Payrollcal::pre($get_payroll_condition_value_arr);die;
                            $output .= "if ".$get_payroll_condition_value_arr[0]." then ".$p['code']." = ".$get_payroll_condition_value_arr[1];
                        }else{
                            $showValue=$p['value_formula'];
                            $output .= $showValue;
                        }
                    } else {
