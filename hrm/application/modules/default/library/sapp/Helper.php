...
//                            }
                ...
                if($tot_rec_cnt > 0)
                {
                    for($i=2; $i <= $sizeOfWorksheet; $i++ )
                    {
                        ...
                        sapp_Schoolteacher::_generate_log("user data for HRM DB : ". json_encode($user_data));
                        $user_id = $usersModel->SaveorUpdateUserData($user_data, '');
                        sapp_Schoolteacher::_generate_log("user id generated at HRM : ".$user_id);
                            sapp_Schoolteacher::_generate_log("user type at HRM : ".$rowData[4]);
                        //===========teacher creation in school start here ==========||
                        if(strtolower($rowData[4])=='teacher' || strtolower($rowData[4])=='librarian'){
                            if(strtolower($rowData[4])=='teacher'){
                                 $roletype='teacher';
                            }else{
                                 $roletype='librarian';
                            }
                            ...
                            $schooolDatabaseUpdateDataArr=[
                                                    "table"=>$roletype,
                                                    "name"=>$rowData[1],
                                                    "last_name"=>(trim($rowData[2])!='')?$rowData[2]:NULL,
                                                    "experience"=>(trim($rowData[14])=="")?1:$rowData[14],
                                                    "specialisation"=>$roletype,
                                                    "email"=>$rowData[5],
                                                    "password"=>$emppassword,
                                                    "cell_phone"=>(trim($rowData[16])!=''?$rowData[16]:'9999999999'),
                                                    "job_title"=>$roletype,
                                                    "emp_id"=>$user_id,
...
+                           sapp_Schoolteacher::_generate_log("user data from HRM to SCHOOL ". json_encode($schooolDatabaseUpdateDataArr));
                            ...
                            sapp_Schoolteacher::_generate_log("user type at HRM nither teacher nor libraryan ");
                        }
