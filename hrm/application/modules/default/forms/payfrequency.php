...
class Default_Form_payfrequency extends Zend_Form
{
	...
	$payslipgenerateday = new Zend_Form_Element_Select('payslipgenerateday');
         $payslipgenerateday->setLabel("Payslip Generate Day")
                 ->setAttrib("id", 'payslipgenerateday')
                 ->setAttrib('name', 'payslipgenerateday')
                 ->setMultiOptions($this->getSelectBoxDataArr())
                ->setRequired(TRUE)
                 ->addValidator('NotEmpty',FALSE,array('messages' => 'Please select days from dropdown.'));	
		...
		$submit->setLabel('Save');
		 ...
		 //$this->addElements(array($id,$freqtype,$freqshortcode,$description,$submit));
                 $this->addElements(array($id,$freqtype,$freqshortcode,$description,$submit,$payslipgenerateday));
         $this->setElementDecorators(array('ViewHelper')); 
	}
        
        private function getSelectBoxDataArr(){
             $dataArr=array();
             for($i=1;$i<31;$i++){
                 $dataArr[$i]=$i;
             }
