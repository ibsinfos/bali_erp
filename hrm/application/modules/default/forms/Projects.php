	...
	public function init()
	{
        ...
        $project_name->addValidator(new Zend_Validate_Db_NoRecordExists(
                                              array('table'=>'tm_projects',
                                                     'field'=>'project_name',
                                                     'exclude'=>'id!="'.Zend_Controller_Front::getInstance()->getRequest()->getParam('id').'" and is_active=1 AND school_id = "'.$school_id.'"',    
                                                 ) )  
                                    );
		...
		$client->addValidator(new Zend_Validate_Db_RecordExists(
										array('table' => 'tm_clients',
                                        		'field' => 'id',
                                                'exclude'=>'is_active = 1 AND school_id = "'.$school_id.'"',
										)));
