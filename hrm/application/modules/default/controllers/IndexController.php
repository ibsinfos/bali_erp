...
//            $this->view->messages = $this->_helper->flashMessenger->getMessages();
	...
	public function loginpopupsaveAction()
	{
            ...
            $options=array();
            if($this->getRequest()->getParam('mode') == 1) {
                ...
                if(!isset($_COOKIE['username'])||!isset($_COOKIE['password']))
                {
                     ...
                     $url="http://".$_SERVER['HTTP_HOST']."/".$url_arr[1]."/index.php?login/logout";
                     $this->_redirect($url);
                }  
                 $emailParam = $username;
                 $opt= array (
...
//            echo $options['username']; echo '|||||||||'; echo $options['user_password']; echo '|||||||||'; echo sha1($password); echo '|||||||||';
//            echo md5($options['user_password']); die;
            $arrValue = array();

            $usersModel = new Default_Model_Users();
            $userData = $usersModel->isActiveUser($options['username'],$options['user_password']);
            ...
            if(!$check)
            {
                $userStatusArr = $usersModel->getActiveStatus($options['username'],$options['user_password']);
             ...
             */
            $userDateOfJoining = $usersModel->getUserDateOfJoining($options['username'],$options['user_password']);
...
//                            $this->_redirect('index');
            ...
            try
            {
                ...
                if ($user->isLdapUser(sapp_Global::escapeString($options['username']),$options['user_password'])) {
                ...
                if ($result->isValid()) {
                    $admin_data = $user->getUserObject($options['username'],$options['user_password']);
                    ...
                    **/
                    ...
                    //echo $tm_role = Zend_Registry::get('tm_role');die;
                    /***
                ...
                { 
                    $this->_helper->getHelper("FlashMessenger")->addMessage("The username or password you entered is incorrect.");
//                    $this->_redirect('../index.php?admin/dashboard');
                    $this->view->messages = $this->_helper->flashMessenger->getMessages();
                    $this->render('errorpage');
                }
            }
				...
				foreach($newarr as $deparr)
				{
					$where.= " id!= $deparr AND ";
                                ...
                                if(!empty($school_id) && $school_id>0) {
                                    $where .= " d.school_id = '".$school_id."' AND ";
                                }
	...
	public function welcomeAction()
	{
		...
		if($auth->hasIdentity())
		{
			...
			$isOrganizationHead = $auth->getStorage()->read()->is_orghead;
                ...
                //$storage = $auth->getStorage()->read(); echo '<pre>'; print_r($storage); die;
