...
//end of inactiveusers action.
    ...
    public function preparepayslipAction() {//echo 'fdg'; die;
        $this->_helper->viewRenderer->setNoRender(true);
        $this->_helper->layout()->disableLayout();
        //die("calling");
        ...
        $alllEmployee = $payrollPayslip->get_all_employee();
        ...
        //sapp_Payrollcal::pre($date);die;

        if(date('j')!=$date){
            sapp_Payrollcal::pre("cDate : ".date('j')." ---- "."generate date = ".$date);
            return FALSE;
        }

        
        //$payrollPayslipCateggoryDataArray=array('payroll_category_id'=>1,'formula'=>"10%HRA",'value'=>5000,'payroll_payslip_id'=>10);
        //$this->pre($payrollPayslipCateggoryDataArray);
        //$payrollPayslipDetailsModel->add($payrollPayslipCateggoryDataArray);
        
        foreach ($alllEmployee As $key => $val) {
            //sapp_Payrollcal::pre($val);
            if ($val['emprole'] != 10) {
                //continue;
            }
            /*if($val['user_id']!=99){
            ...
            foreach ($allEarningDeduction AS $key=>$value){
                ...
                if ($value['code'] == "BAS") {
                    ...
                    if ($value['value_type_id'] == 1) { 
                        sapp_Payrollcal::pre($value);                      //formula
                        $formula = $value['value_formula'];
                        //sapp_Payrollcal::pre($formula);die;
                        $string = str_replace("%", "/100", $formula);
                        //sapp_Payrollcal::pre($string);die;
                        //$this->pre($salaryCategory);die;
                        $salaryCategoryKeyArr= array_keys($salaryCategory);
                        $salaryCategoryValArr= array_values ($salaryCategory);
                        //$this->pre($salaryCategoryValArr);die;
                        ...
                        $calculatedValue = $payrollCal->e("y($salaryCategoryValStr)");//die;
                        //$calculatedValue = $payrollCal->e($str);
                        //sapp_Payrollcal::pre('$calculatedValue : - '.$calculatedValue);die;
                        ...
                        //sapp_Payrollcal::pre($allEarningDeduction);die;
                        ...
                        //$conditionAmount= $payrollcategoryModel->get_final_charges_of_payroll_category_by_code($value['code'], $salaryCategory['BAS']);
                        sapp_Payrollcal::pre('$conditionAmount : '.$conditionAmount);
                        //sapp_Payrollcal::pre('$conditionAmount : '.$conditionAmount);die;
                        $allEarningDeduction[$key]['value']=$conditionAmount;
                        //sapp_Payrollcal::pre($allEarningDeduction[$key]['value']);die;
                        $tempSalaryCategoryalue=$allEarningDeduction[$key]['value'];
                        $salaryCategory[$value['code']]=$tempSalaryCategoryalue;
                    }else{ //$this->pre("come at 731 at else part");die;
                        ///fix
            ...
            //die("ZZZZZZZZZZZ");
            //$this->pre('<br> YYYYYYYYYYYYY<br>'.$totalSal);
            //$this->pre($allEarningDeduction);//die;
            ...
            if($totalSal>0){
                //$this->pre("more than 0");
            }else{
                //$this->pre("less than 0");
