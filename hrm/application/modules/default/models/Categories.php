	...
	**/
	public function getCategories($con,$sort='', $by='', $pageNo='', $perPage='',$searchQuery='')
	{
		...
		try
		{
			...
			if($searchQuery)
				$where .= " AND ".$searchQuery;
                        if(!empty($school_id) && $school_id > 0) {
                            ...
                            $swhere = " AND d.school_id = '".$school_id."'";
                        }
			...
			{
				$qry = 'select d.category_id,count(d.id) as doccnt  from main_pd_documents d where d.isactive = 1 '.$swhere.' group by d.category_id'; 					
