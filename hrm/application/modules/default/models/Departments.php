...
//			$where .= ' AND d.unitid='.$unitid.' ';
                ...
                if(!empty($school_id) && $school_id > 0) {
                    $where .= " AND d.school_id = '".$school_id."'";
                }
	...
	public function getGrid($sort,$by,$perPage,$pageNo,$searchData,$call,$dashboardcall,$unitId,$a='',$b='',$c='')
	{
		...
		$objName = 'departments';
		$tableFields = array('action'=>'Action','deptname' => 'Name','deptcode' =>'Code','startdate'=>'Started On','depthead'=>'Department Head','timezone'=>'Time Zone');
	...
	public function checkCodeDuplicates($code,$id)
	{
            ...
            if($id) $row = $this->fetchRow("deptcode = '".$code."' AND id <> ".$id.' AND isactive=1'.$where); 
            else  { $row = $this->fetchRow("deptcode = '".$code."' AND isactive=1".$where); }
            if(!$row){
                return false;
            }else{
                return true;
	...
	public function getUniqueDepartmentList($deptidarr,$businessunitid)
	{
        ...
        $query = "select d.id,d.deptname from main_departments d where d.isactive = 1 $where";
	...
	public function checkExistance($deptname, $unitid,$id)
	{
		...
		$query = "select count(*) as count from main_departments where isactive = 1 and deptname = '".$deptname."' ".$where;
        $result = $db->query($query)->fetchAll();
