...
class Default_Model_Users extends Zend_Db_Table_Abstract
{
	 ...
	 * @return boolean
	 */
	public function isLdapUser($username,$pass) {
			
		return false;
	}

	public function getUserObject($username,$pass)
	{
		if (empty($username))
		{
			return false;
       ...
       ->where("u.isactive = 1 and (u.employeeId = ? OR u.emailaddress = ?) and emppassword = '".md5($pass)."'",array($username));
	...
	public function isActiveUser($corpEmail,$pass){
		try
		{
			$db = Zend_Db_Table::getDefaultAdapter();
			...
			->where("a.isactive = 1 AND r.isactive = 1 AND a.emptemplock = 0 AND (a.employeeId = ? OR a.emailaddress = ?) AND emppassword = '".md5($pass)."'",array($corpEmail));
	...
	public function getActiveStatus($corpEmail,$pass)
	{
		try
		{
			...
			->where("(u.employeeId = ? OR u.emailaddress = ?) AND emppassword = '".md5($pass)."'",array($corpEmail));
	...
	public function getUserDateOfJoining($corpEmail,$pass)
	{
		try
		{
			$userData = $this->select()->setIntegrityCheck(false)
			...
			->where("(u.employeeId = ? OR u.emailaddress = ?) AND u.emppassword = '".md5($pass)."'",array($corpEmail));
            ...
            if($employee_group != "")
            {
                if($employee_group == MANAGEMENT_GROUP)
                {
                    $qry_str = " SELECT u.id, concat(u.userfullname,if(j.jobtitlename is null,'',concat(' , ',j.jobtitlename))) as name,u.profileimg 
                                ...
                                INNER JOIN main_employees e ON u.id = e.user_id 
                                LEFT join main_jobtitles j on j.id = e.jobtitle_id 
                                WHERE  r.group_id IN (1)  AND u.userstatus='old' AND u.isactive=1 AND r.isactive=1 ".$where;
                    if($dept_id == '')
                    {
                        $qry = $qry_str;
                        if($employee_id != "")
                ...
                else 
                {
                    if($dept_id != '')
                    {
                        $qry = "select * from ((SELECT u.id, concat(u.userfullname,if(j.jobtitlename is null,'',concat(' , ',j.jobtitlename))) as name,u.profileimg 
                                ...
                                INNER JOIN main_employees e ON u.id = e.user_id 
                                left join main_jobtitles j on j.id = e.jobtitle_id 
                                WHERE e.department_id = ".$dept_id."".$where;
                        ...
                        $qry .= "   r.group_id = ".MANAGEMENT_GROUP ." AND u.isactive=1 AND r.isactive=1 ".$where.")";
