...
class Default_Model_Roles extends Zend_Db_Table_Abstract
{
	...
	public function getRolesData($sort, $by, $pageNo, $perPage,$searchQuery)
	{
        ...
        public function getRolesDataforpayroll($sort, $by, $pageNo, $perPage,$searchQuery)
	{
		$where = "r.isactive = 1";
		...
		$roleData = $this->select('gernerate payslip as payslip')
		->setIntegrityCheck(false)
		->from(array('r'=>$this->_name),array('r.*',))
		->joinInner(array('g'=>'main_groups'), "g.id = r.group_id and g.isactive = 1",array('group_name'=>'g.group_name'))
		->where($where)
...
public function getRolesList_EMP($con='')
	...
	public function getGrid($sort,$by,$perPage,$pageNo,$searchData,$call,$dashboardcall,$p2,$p3,$p4,$p5)
	{
		...
		$tableFields = array('action'=>'Action',
                             ...
                             //$tablecontent = $this->getRolesDataforpayroll($sort, $by, $pageNo, $perPage,$searchQuery);
        ...
        public function getGridforpayroll($sort,$by,$perPage,$pageNo,$searchData,$call,$dashboardcall,$p2,$p3,$p4,$p5)
	{
		$group_model = new Default_Model_Groups();
		$searchQuery = '';
		$searchArray = array();
    ...
    function getAllRollDataForList(){
        $roleData = $this->select('gernerate payslip as payslip')
		->setIntegrityCheck(false)
		->from(array('r'=>$this->_name),array('r.*',))
		->joinInner(array('g'=>'main_groups'), "g.id = r.group_id and g.isactive = 1",array('group_name'=>'g.group_name'))
		//->where($where)
		->order("r.modifieddate DESC");
		//->limitPage($pageNo, $perPage);
        $db = Zend_Db_Table::getDefaultAdapter();
        $rs=$db->fetchAssoc($roleData);
		return $rs;
    }
    
    public function getAllEmployeIdByRoleId($roleId){
        $sql="SELECT id FROM `main_users` WHERE emprole='".$roleId."'";
        //die($sql);die;
        $db = Zend_Db_Table::getDefaultAdapter();
    ...
    public function getPayrollGrid($sort,$by,$perPage,$pageNo,$searchData,$call,$dashboardcall,$p2,$p3,$p4,$p5)
    {
        $searchQuery = '';
        $searchArray = array();
        $data = array();
        ...
        $tablecontent = $this->getRolesDataforpayrollgrid($sort, $by, $pageNo, $perPage,$searchQuery);
    ...
    public function getRolesDataforpayrollgrid($sort, $by, $pageNo, $perPage,$searchQuery)
    {
        ...
        if($searchQuery != '')
            $where .= " ".$searchQuery;
        if(trim($where)!=""){
            $where .=" AND g.isactive = 1";
        }else{
            $where .=" g.isactive = 1";
        }
        $where .=" AND r.isactive = 1";
        //echo //$where;die;
     $payrollRoleData = $this->select()
		->setIntegrityCheck(false)
