...
class Default_Model_Requisition extends Zend_Db_Table_Abstract
{
	...
	$auth = Zend_Auth::getInstance();
		...
		**/
		$len = strlen($prefix)+1;
        $db = Zend_Db_Table::getDefaultAdapter();
        $query = "select max(cast(substr(requisition_code,".$len.") as unsigned))  req_id from main_requisition WHERE school_id = '".$school_id."'";
        $result = $db->query($query);
        $row = $result->fetch();
        $req_id = $row['req_id'];
	...
	public function getEmpStatusOptions($id='')
	{
        ...
        public function getStatusOptionsForRequi()
        {
            ...
            $query = "select mn.id,tb.employemnt_status 
                      ...
                      where mn.isactive = 1 and tb.id not in (5,7,8,9,10) and mn.school_id = '".$school_id."' order by tb.employemnt_status;"; //mn.isactive = 1 and 
	...
	**/
	...
	{	
		...
		if($id!='')
		{
			$titlesData = $db->query("select * from main_jobtitles where isactive = 1 AND id=".$id." AND school_id = '".$school_id."' order by jobtitlename;"); //isactive = 1 AND 
			return $data = $titlesData->fetch();
		}
		else
		{
			$titlesData = $db->query("select * from main_jobtitles where isactive = 1 AND school_id = '".$school_id."' order by jobtitlename;");//where isactive = 1
