...
class Default_Model_Employee extends Zend_Db_Table_Abstract
{
	...
	*/
    ...
    public function getGrid($sort,$by,$perPage,$pageNo,$searchData,$call,$dashboardcall,$exParam1='',$exParam2='',$exParam3='',$exParam4='',$school_id)
    {
        $searchQuery = '';
        $tablecontent = '';
        $emptyroles=0;
        ...
        if($searchData != '' && $searchData!='undefined')
        {
            ...
            if($school_id!='')
            $searchQuery .= "school_id = '".$school_id."' AND ";
            $searchQuery = rtrim($searchQuery," AND");					
        } else {
            if($school_id!='')
            $searchQuery = "school_id = '".$school_id."'";
        } 
        $objName = 'employee';
     ...
     public function getGridforpayroll($sort,$by,$perPage,$pageNo,$searchData,$call,$dashboardcall,$exParam1='',$exParam2='',$exParam3='',$exParam4='')
    {
        $searchQuery = '';
        $tablecontent = '';
        $emptyroles=0;
        ...
        $tableFields = array('firstname'=>'First Name','lastname'=>'Last Name',
                             'employeeId' =>'Employee ID','extn'=>'Work Phone',
	...
	public function getCurrentOrgHead()
	{
            ...
            $db = Zend_Db_Table::getDefaultAdapter();
            $query = "select user_id from main_employees where is_orghead = 1 and isactive = 1".$where;
            $result = $db->query($query);
            $emp_arr = array();
            $emp_arr = $result->fetchAll();
            return $emp_arr;
...
//			$where .= ' AND e.businessunit_id = '.$bunitid.'';
    ...
    public function checkphoneexist($phone,$id){ 
        ...
        $where_condition = '';
        if($id){
            $where_condition = " AND user_id!='".$id."'";
        }
        $query = "select count(id) as emp_count from main_employees where office_number='".$phone."' $where_condition ";
        $result = $db->query($query);
        $data = $result->fetch();
        return $data['emp_count'];
    }
    ...
    public function getUserRole_by_id($role_id)
	{	
            
		$db = Zend_Db_Table::getDefaultAdapter();
		
		$usersData = $db->query("SELECT * FROM main_roles As r WHERE `id`='$role_id'");
        ...
        $tablecontent = $this->getDataForGridPayslipEmployee($sort, $by, $pageNo, $perPage,$searchQuery);
        ...
        //echo '<pre>';print_r($dataTmp); die;
        return $dataTmp; 
    }
    
    function getDataForGridPayslipEmployee($sort, $by, $pageNo, $perPage,$searchQuery){
        /*$sql="SELECT mes.*,(SELECT pp.status FROM `payroll_payslip` AS pp WHERE MONTH(pp.`generate_date`)=MONTH(CURRENT_DATE()) AND YEAR(pp.`generate_date`)=YEAR(CURRENT_DATE()) AND pp.`emp_id`=mes.user_id ORDER BY pp.id DESC LIMIT 0,1) AS paysllip_status FROM main_employees_summary AS mes ";
        $employeesData = $this->select()
                                ->setIntegrityCheck(false)	                                
                                ->from(array('e' => 'main_employees_summary'),
                                        array('*','id'=>'e.user_id','extn'=>new Zend_Db_Expr('case when e.extension_number is not null then concat(e.office_number," (ext ",e.extension_number,")") when e.extension_number is null then e.office_number end'),'astatus'=> new Zend_Db_Expr('case when e.isactive = 0 then "Inactive" when e.isactive = 1 then "Active" when e.isactive = 2 then "Resigned"  when e.isactive = 3 then "Left" when e.isactive = 4 then "Suspended" end')
     ...
     /*$payrollEmployeeData = $this->select()
		...
		->joinInner(array('g'=>'main_groups'), "g.id = r.group_id",
                            'g.group_name')
		->where($where)
		->order("$by $sort")
