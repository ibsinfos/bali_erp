...
class Default_Model_Appraisalratings extends Zend_Db_Table_Abstract
{
	...
	public function getAppraisalRatingsData($sort, $by, $pageNo, $perPage,$searchQuery,$implementationflag)
	{
            ...
            $where = " i.isactive=1 AND r.isactive=1 ";
		...
		$appraisalRatingsData = $this->select()
                           ...
                           ->joinLeft(array('d'=>'main_departments'), 'i.department_id = d.id', array('d.*'))
                           ->where($where)
    					   ->order("$by $sort")
    					   ->group('i.id') 
	...
	{		
		...
		$tableFields = array('action'=>'Action','appr_period'=>'Appraisal Period','deptname' => 'Department','rating_type'=>'Rating Type','status'=>'Appraisal Status','appraisal_process_status'=>'Process Status');
