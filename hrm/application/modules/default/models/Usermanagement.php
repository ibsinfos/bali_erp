...
class Default_Model_Usermanagement extends Zend_Db_Table_Abstract
{
    ...
    public function SaveorUpdateUserData($data, $where)
    {
        ...
        else 
        {
            ...
            //echo $querystr;die;
            //Zend_Debug::dump($querystr);die;
            $id=$this->getAdapter()->lastInsertId($this->_name);
            return $id;
        }
    }
    public function replicateTeachers($data,$where)
    {
        //print_r($data); die;
       // use Zend\Db\Sql\Sql;
        ...
        $dbAdapter->insert('teacher', $data);
        Zend_Debug::dump($dbAdapter->getProfiler()->getLastQueryProfile()->getQuery());
        Zend_Debug::dump($dbAdapter->getProfiler()->getLastQueryProfile()->getQueryParams());
        $dbAdapter->getProfiler()->setEnabled(false);
        /* echo "Here";
          exit; */
