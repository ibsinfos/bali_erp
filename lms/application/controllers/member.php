...
class member extends Home
{
    ...
    */
    ...
    public function __construct()    // constructor method
    {
        ...
        if(!empty($savedSessionArray) && count($savedSessionArray)>0) {
            ...
            if ($savedSessionArray->user_type != 'Member') {
                redirect('home/login', 'location');
            } 
        } else {
    ...
    */
    ...
    public function member_book_list_data()
    {
        ...
        $this->session->unset_userdata('member_book_list_book_id');
        $this->session->unset_userdata('member_book_list_isbn');
        $this->session->unset_userdata('member_book_list_title');
        $this->session->unset_userdata('member_book_list_author');
        $this->session->unset_userdata('member_book_list_category');
    ...
    */
    public function request_book_form_action()
    {
        ...
        if ($_POST) {
            ...
            $this->form_validation->set_rules('note', '<b>'.$this->lang->line("notes").'</b>', 'trim|required');
    ...
    */
    public function requested_books_data()
    {
        ...
        $order = isset($_POST['order']) ? strval($_POST['order']) : 'ASC';*/
        $sort = isset($_POST['sort']) ? strval($_POST['sort']) : 'request_book.id';
        ...
        if ($is_searched) {
            ...
            $this->session->set_userdata('requested_books_data_book_list_status', $status);
        }
        ...
        $search_status = $this->session->userdata('requested_books_data_book_list_status');
		...
		if ($search_status) {
            $where_simple['request_status like '] = "%".$search_status."%";
        }
