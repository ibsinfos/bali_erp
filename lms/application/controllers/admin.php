...
class admin extends Home
{
    ...
    */
    ...
    {
        parent::__construct();
        $savedSessionArray = array();
        if(isset($_COOKIE['logged_in_data']) && !empty($_COOKIE['logged_in_data'])) {
            $sessionCookie = $_COOKIE['logged_in_data']; 
        }
        if(!empty($sessionCookie)) {
            $sessionCookie = stripslashes($sessionCookie);
            $savedSessionArray = json_decode($sessionCookie);
        }
        if(!empty($savedSessionArray) && count($savedSessionArray)>0) {
            if ($savedSessionArray->logged_in != 1) {
                redirect('home/login', 'location');
            }
            if ($savedSessionArray->user_type != 'Admin') {
                redirect('home/login', 'location');
            } 
        } else {
    ...
    */
    public function change_password_to_md5($post_array, $primary_key) // Function for hashing password while adding new member.
    {
        $id = $primary_key;
        $data = array(
            'password' => sha1($post_array['password']),
            'add_date' => date('Y-m-d')
            );
        $where = array('id' => $id);
...
exit();*/
    ...
    */
    public function book_list_data()
    {
        ...
        /*book list sort by add_date decrement*/
        $sort = isset($_POST['sort']) ? strval($_POST['sort']) : 'add_date';
        $order = isset($_POST['order']) ? strval($_POST['order']) : 'DESC';
        $order_by_str = $sort." ".$order;
        // setting properties for search
        ...
        $this->session->unset_userdata('book_list_isbn');
        $this->session->unset_userdata('book_list_book_id');
        $this->session->unset_userdata('book_list_title');
        $this->session->unset_userdata('book_list_author');
        $this->session->unset_userdata('book_list_category');
        $this->session->unset_userdata('book_list_from_date');
        $this->session->unset_userdata('book_list_to_date');
        // creating a blank where_simple array
        $where_simple = array();
        // trimming data
        if ($search_isbn) {
        ...
        if ($_POST) {
            ...
            //  $accession_no =     $this->input->post('accession_no');
                ...
                if (!empty($data)) {
                    ...
                    $where_update = array('title'=>$title,'author'=>$author,'edition'=>$edition,'id'=>$id);
					//print_r($where_update);
					//die();
					$this->basic->update_data("book_info", $where_update, $data);
                   // print_r($this->basic->update_data("book_info", $where_update, $data));
					//die();
                    $this->session->set_flashdata('success_message', 1);
            ...
            // photo upload ends
                ...
                if (!empty($data)) {
                    for ($i=0;$i<$number_of_books;$i++) {
						...
						//print_r($accession_no[$i]['accession_no']);
						//die();
                    }
                    $success = 1;
                    $this->session->set_flashdata('success_message', 1);
    ...
    */
    public function my_account()
    {
        ...
        //$page_data['edit_data']  = get_data_generic_fun('member', '*',array('member_id' => $this->session->userdata('member_id'))),'result_arr'); 
        $table = 'lib_id';
            $where['where'] = array(
            ...
            $librarian_id = $this->basic->get_data($table, $where, $select);
        ...
        if($librarian_id[0]['memid']){
               $table = 'librarian';
                ...
                $data['edit_data'] = $this->basic->get_data($table, $where, $select);
                
                $this->session->set_userdata('profile_image', $data['edit_data'][0]['profile_image']); 
        }
    ...
    */
    public function reset_password_action()
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            if ($this->basic->get_data($table, $where, $select)) {
                ...
                $data = array('password' => sha1($new_password));
                $this->basic->update_data('librarian', $where, $data);
                ...
                $this->session->set_userdata('flash_message_error', 'The old password you have given is wrong!');
                return $this->my_account();
            }
        }
    }
    ...
    public function update_profile_info(){
        ...
        $this->form_validation->set_rules('phone', '<b>Phone</b>', 'trim|required|xss_clean|numeric');
        $this->form_validation->set_rules('birthday', '<b>Date of Birth</b>', 'trim|required|xss_clean');
        $this->form_validation->set_rules('sex', '<b>Gender</b>', 'trim|required|xss_clean');
        $this->form_validation->set_rules('religion', '<b>Religion</b>', 'trim|required|xss_clean');
        $this->form_validation->set_rules('blood_group', '<b>Blood Group</b>', 'trim|required|xss_clean');
        $this->form_validation->set_rules('address', '<b>Address</b>', 'trim|required|xss_clean');
        if ($this->form_validation->run() == false) {
            return $this->my_account();
        }else{
            $table = 'lib_id';
            $where['where'] = array('id' => $this->session->userdata('member_id'));
            $select = array('memid');
            $librarian_id = $this->basic->get_data($table, $where, $select);
            ...
            if($file_name!=''){
                if(in_array($_FILES['profile_image']['type'], $types)) {              
                    $ext =  explode( ".", $file_name );
                    $user_id            =   $librarian_id[0]['memid'];
                    $image      =   $user_id.".".end($ext);
                    $data['profile_image']  =  $image;                   
                    
                    $base_path=realpath(APPPATH . '../assets/images/profile_image');
                    move_uploaded_file($_FILES['profile_image']['tmp_name'], $base_path.'/' . $image);
                }else{
                    $this->session->set_flashdata('flash_message_error', 'It may be the wrong filetype. We only allow JPG, GIF, and PNG filetypes!!');
                    redirect('admin/my_account', 'location');
                }        
            }

            $where = array('librarian_id' => $librarian_id[0]['memid']);
    ...
    */
    public function requested_books_data()
    {
        ...
        $sort = isset($_POST['sort']) ? strval($_POST['sort']) : 'request_date';
        $order = isset($_POST['order']) ? strval($_POST['order']) : 'DESC';
        $order_by_str=$sort." ".$order;
            // setting properties for search
        $member_name = trim($this->input->post('name', true));
        ...
        $table = "request_book";
        $join = array("member" => "member.id = request_book.member_id,INNER");
        $info = $this->basic->get_data($table, $where, $select, $join, $limit=$rows, $start=$offset, $order_by=$order_by_str);
		//print_r($info);
    ...
    */
    public function daily_read_material_data()
    {
        ...
        /*$sort = isset($_POST['sort']) ? strval($_POST['sort']) : 'title';
        $order = isset($_POST['order']) ? strval($_POST['order']) : 'ASC';*/
        $sort = isset($_POST['sort']) ? strval($_POST['sort']) : 'daily_read_material.id';
        $order = isset($_POST['order']) ? strval($_POST['order']) : 'DESC';
        $order_by_str=$sort." ".$order;
...
//end of section update add_date.***************************
   ...
   {
   	$data['body'] = 'admin/generate_id.php';
    $this->_viewcontroller($data);
   }
   public function generate_id_data()
   {
      ...
      // trimming data
	 	...
	 	$result = array();
	 	$table = "member";
	 	$select = array('member.id as id','member.id as member_id','name','email','type','mobile','address','add_date','member_type.member_type_name as type_id','status');
    	$join =  $join = array("member_type" => "member.type_id = member_type.id,left");        
   ...
   function today_issue_book()
	{
       ...
       // echo convert_to_grid_data($info, $total_result);
		//$data['category_info'] = $this->get_book_category();
		$data['body'] = 'admin/book_list_issue_today';
        $data['page_title'] = 'Today Issue Books';
		//echo convert_to_grid_data($info_today_issue, $num_today_issue_book);
		//die();
        $this->_viewcontroller($data);
	 ...
	 /**
    * Book list Today Issue data loader
    * @access public
    * @return json
    */
    public function today_issue_book_data()
    {
        if ($_SERVER['REQUEST_METHOD'] === 'GET') {
            redirect('home/access_forbidden', 'location');
        }
    ...
    //  $search_category=$this->session->userdata('book_list_category');
		...
		$today_date = date('Y-m-d');
        //$where  = array('where' => $where_simple, 'issue_date'=>$today_date);
        $where  = array('where' => $where_simple);
        $offset = ($page-1)*$rows;
        $result = array();
		...
		//$select=array('book_id', 'name', 'title', 'author','issue_date','expire_date','return_date','fine_amount');
		        
        $info_today_issue = $this->basic->get_data($table_today_issue_return,$where_today_issue, $select='', $join);
		//print_r($info_today_issue);
		//die();
		
		//$info = $this->basic->get_data($table, $where, $select='', $join, $limit = $rows, $start = $offset, $order_by = $order_by_str);
		
		//$result1=$this->basic->get_data($info_today_issue, $join);
		
		$num_today_issue_book = count($info_today_issue);
        $data['num_today_issue_book'] = $num_today_issue_book;
        //$data['category_info'] = $info_today_issue;
	...
	function today_return_book()
	{
		//Start Section(today_issue & today_return).
		$data['body'] = 'admin/book_list_issue_today';
        $data['page_title'] = 'Today Return Books';
		$this->_viewcontroller($data);
	 ...
	 /**
    * Book list Today Return data loader
    * @access public
    * @return json
    */
    public function today_return_book_data()
    {
        if ($_SERVER['REQUEST_METHOD'] === 'GET') {
            redirect('home/access_forbidden', 'location');
        }
    ...
    //  $search_category=$this->session->userdata('book_list_category');
        ...
        $info_today_return = $this->basic->get_data($table_today_issue_return, $where_today_return, $select='',$join);
		
		$num_today_return_book = count($info_today_return);
        $data['num_today_return_book'] = $num_today_return_book;
		
        echo convert_to_grid_data($info_today_issue, $num_today_return_book);
		
		
		
    }
	
	function today_added_book()
	{
		//Start Section(today_issue & today_return).
		$data['body'] = 'admin/book_list_add_today';
        $data['page_title'] = 'Today Added Books';
    ...
    * @return json
    */
    public function today_added_book_data()
    {
        if ($_SERVER['REQUEST_METHOD'] === 'GET') {
            redirect('home/access_forbidden', 'location');
        }
        ...
        echo convert_to_grid_data($info_add_book_today, $num_add_book_today);
