        ...
        if ($_POST) {
            ...
            if ($this->form_validation->run() == false) {
                ...
                $this->basic->import_dump($dump_sql_path);
                  ...
                  $app_password = sha1($app_password);
    ...
    */
    ...
    { 
        ...
        if ($this->session->userdata('logged_in') == 1 && $this->session->userdata('user_type') == 'Member') {
         ...
         //Captcha starts
        $path = base_url();
        $MainPath= explode('lms', $path);

        $path2 = FCPATH;
        $MainPath2= explode('lms', $path2);

        $this->load->helper('captcha');
        $CaptchaConfig = array('img_path'=> $MainPath2[0].'uploads/captcha/', 'img_url'=>$MainPath[0].'uploads/captcha/', 'img_width'=>150, 'img_height'=>40, 'expiration'=>600);
        ...
        if ($this->form_validation->run() == false) {
            ...
            if ($count == 0) {
                ...
                $info = $this->basic->get_data('student', $where, $select = '', $join = '', $limit = '', $start = '', $order_by = '', $group_by = '', $num_rows = 1);
                ...
                if($user_type=='Admin'){
                    ...
                    if($librarian_id[0]['memid']){
                        ...
                        $data = $this->basic->get_data($table, $where, $select); 
    ...
    */
    ...
    public function recovery_check()
    {
        ...
        if ($_POST) {
            ...
            $newp = sha1($this->input->post('newp', true));
            $conf = sha1($this->input->post('conf', true));
