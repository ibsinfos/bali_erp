...
class Donation extends CI_Controller {
    ...
    function __construct() {
        ...
        $this->load->model("fees/Donation_model");
    ...
    public function index() {
        if(($this->session->userdata('school_admin_login') == 1) || ($this->session->userdata('accountant_login') == 1)){
            ...
            $page_data['page_name'] = 'fees/donation/index';
            $page_data['page_title'] = 'donation';
            $page_data['data'] = $this->Donation_model->get_donation_data();        
    ...
    function add(){
        if(($this->session->userdata('school_admin_login') == 1) || ($this->session->userdata('accountant_login') == 1)){
            $this->form_validation->set_rules('donator_name', 'Donator Name', 'trim|required');
            $this->form_validation->set_rules('description', 'Description', 'trim|required');
            $this->form_validation->set_rules('donation_date', 'Donation Date', 'trim|required');
            $this->form_validation->set_rules('amount', 'Amount', 'trim|required');
             ...
             if ($this->form_validation->run() == FALSE){
                ...
                $id = $this->Donation_model->add($data);
                if($id){
                    $this->session->set_flashdata('flash_message','Donation has been added successfully');
                }else{
                    $this->session->set_flashdata('flash_message_error', get_phrase('something_went_wrong.'));
                }
             }
             redirect(base_url() . 'index.php?fees/donation', 'refresh');
        }else{
            redirect(base_url(), 'refresh');
        }
    }
    ...
    public function delete($id){
         if(($this->session->userdata('school_admin_login') == 1) || ($this->session->userdata('accountant_login') == 1)){
            $this->Donation_model->udpate(array('donation_status'=> '0'), $id); 
            $this->session->set_flashdata('flash_message', get_phrase('donation_has_been_deleted_successfully.'));        
            redirect(base_url() . 'index.php?fees/donation', 'refresh');
        }else{
            redirect(base_url(), 'refresh');
        }
