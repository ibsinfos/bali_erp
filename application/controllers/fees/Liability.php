...
class Liability extends CI_Controller {
    ...
    function __construct() {
        ...
        $this->load->model("fees/Liability_model");
    ...
    public function index() {
        if(($this->session->userdata('school_admin_login') == 1) || ($this->session->userdata('accountant_login') == 1)){
            ...
            $page_data['page_name'] = 'fees/liability/index';
            $page_data['page_title'] = 'liability';
            $page_data['data'] = $this->Liability_model->get_liability();        
        ...
        if(($this->session->userdata('school_admin_login') == 1) || ($this->session->userdata('accountant_login') == 1)){
            ...
            $this->form_validation->set_rules('description', 'Description', 'trim|required');
             ...
             if ($this->form_validation->run() == FALSE){
                ...
                $id = $this->Liability_model->add($data);
                if($id){
                    $this->session->set_flashdata('flash_message','Liability has been added successfully');
                }else{
                    $this->session->set_flashdata('flash_message_error', get_phrase('something_went_wrong.'));
                }
             }
             redirect(base_url() . 'index.php?fees/liability', 'refresh');
        }else{
            redirect(base_url(), 'refresh');
        } 
    }
        ...
        if(($this->session->userdata('school_admin_login') == 1) || ($this->session->userdata('accountant_login') == 1)){
            ...
            if($param1!=''){
                if ($this->form_validation->run() == FALSE){
                    ...
                    $data = $this->input->post();    
                    $this->Liability_model->do_update($data, $param1);
                    $this->session->set_flashdata('flash_message','Liability has been updated successfully');
                }
                redirect(base_url() . 'index.php?fees/liability', 'refresh');
            }else{
                redirect(base_url() . 'index.php?fees/liability', 'refresh');    
    ...
    public function delete($id=''){
        if(($this->session->userdata('school_admin_login') == 1) || ($this->session->userdata('accountant_login') == 1)){
            $this->Liability_model->do_update(array('fi_liability_status'=> '0'), $id); 
            $this->session->set_flashdata('flash_message', get_phrase('Liability_has_been_deleted_successfully.'));        
            redirect(base_url() . 'index.php?fees/liability', 'refresh');
        }else{
            redirect(base_url(), 'refresh');
        }
