...
class Admin extends CI_Controller {
    ...
    function student_bulk_add($param1 = '') {
        ...
        if ($param1 == 'import_excel') {
            ...
            $fielsdStringMandotary = "name,sex,birthday,caste_category,class_id,section_id,parent_id,address,location,phone,email,card_id,type,icard_no,place_of_birth,country,nationality";
            ...
            if ($someRowError == FALSE) {
                ...
                redirect(base_url() . 'index.php?admin/bulk_upload/' . $this->input->post('class_id'), 'refresh');
            } else {
                //pre($errorMsgArr);die;
                ...
                redirect(base_url() . 'index.php?admin/student_bulk_upload_error' . $this->input->post('class_id'), 'refresh');
            }
        }//ends import excel
        //} 
        $page_data['total_notif_num'] = $this->get_no_of_notication();
    ...
    function student_profile($student_id = '') {
        ...
        $page_data['marks_data'] = $marksData; //pre($page_data['marks_data']); die();
            ...
            foreach ($page_data['subjects'] as $k => $subject):
                ...
                if (count($marks)) {
                    ...
                    foreach ($marks as $mark) {
                        ...
                        $page_data['subjects'][$k]['name'] += $grade['name'];
...
//            $this->form_validation->set_rules('birthday', 'Date of Birth', 'trim|required');
            ...
            $this->form_validation->set_rules('phone', 'Phone Number', 'required|numeric|min_length[7]|max_length[10]');
            $this->form_validation->set_rules('card_id', 'RFID Number', 'trim|required');
//            $this->form_validation->set_rules('password', 'Password', 'trim|required');
            $this->form_validation->set_rules('parent', 'Parent', 'trim|required');
            $this->form_validation->set_rules('lname', 'Last Name', 'trim|required');
            $this->form_validation->set_rules('country', 'Country', 'trim|required');
            $this->form_validation->set_rules('nationality', 'Nationality', 'trim|required');
            $this->form_validation->set_rules('type', 'Identity Card Type|required');
            $this->form_validation->set_rules('icard_no', 'Identity Card Number', 'trim|required');
            ...
            $this->form_validation->set_rules('searchLocation', 'Location', 'trim|required');
            $this->form_validation->set_rules('class_id', 'Class Id', 'trim|required');

            $this->form_validation->set_rules('email', 'Email', 'trim|valid_email|is_unique[student.email]'); //required|
            ...
            if ($this->form_validation->run() == FALSE) {// print_r(validation_errors());die("cp1");
                ...
                $message = array();
                $message['sms_message'] = "Welcome To " . $this->globalSettingsSystemName . " " . $sname . ", Your Child's admission procedure is started.";
...
//                    $this->load->library('upload', $config);
            ...
            if (empty($classArr[0])) {
                ...
                $this->session->set_flashdata('flash_message', get_phrase('data_updated'));
                redirect(base_url() . 'index.php?admin/student_information/' . $classArr[0]->class_id, 'refresh');
            }
        }
    ...
    public function student_fee_configuration($action = '') {
        ...
        if ($action == 'add_student_det') {
            ...
            redirect(base_url() . 'index.php?admin/student_fee_configuration');
        }
        $class_array = $this->Class_model->get_class_array();
        $page_data['total_notif_num'] = $this->get_no_of_notication();
        $page_data['classes'] = $class_array;
    ...
    if ($param1 == 'edit') {
        $this->form_validation->set_rules('fname', 'Father Name', 'trim|required|alpha');
        $this->form_validation->set_rules('flname', 'Father Last Name', 'trim|required|alpha');
        $this->form_validation->set_rules('mname', 'Mother Name', 'trim|required|alpha');
        $this->form_validation->set_rules('mlname', 'Mother Lastname', 'trim|required|alpha');
        ...
        if ($this->form_validation->run() == FALSE) {
            $this->session->set_flashdata('flash_message_error', validation_errors());
            redirect(base_url() . 'index.php?admin/parent/');
        } else {
...
//            redirect(base_url() . 'index.php?admin/parent/', 'refresh');
    ...
    /*     * ***Parent Bulk Upload Function****** */ else if ($param1 == 'import_excel') {
        ...
        if (empty($_FILES['userfile']['name'])) {
            ...
            redirect(base_url() . 'index.php?admin/bulk_upload');
        } else {
            ...
            if (in_array($_FILES['userfile']['type'], $allowed_types)) {
                    ...
                    for ($i = 0; $i < $num_cols; $i++) {    // checking is filds is mandetory or not
                        ...
                        if (in_array($fielsdArr[$i], $fielsdStringMandotaryArr)) {
                            ...
                            if (trim($r[$i]) == "") {
                                ...
                                if ($validPhoneEmailCheck != 'ok' && ($fielsdArr[$i] == 'email' || $fielsdArr[$i] == 'cell_phone')) {
                                    $error = TRUE;
                                    $errorMsgArr[] = $fielsdStringForAdminArr[$i] . " Should be " . $validPhoneEmailCheck . " at row no -" . $errorRowNo;
                                }
                    ...
                    if ($error === FALSE) {
                        ...
                        if ($parent_id > 0) {
                            /* $post = [
                              ...
                              $url = "http://52.29.203.220/School/index.php/?admin/send_common_sms/";
                ...
                if ($someRowError == FALSE) {
                    ...
                    redirect(base_url() . 'index.php?admin/bulk_upload/', 'refresh');
                } else {
                    ...
                    $this->session->set_flashdata('flash_message_error', "Some rows are not uploaded,due to invalid data.");
                    redirect(base_url() . 'index.php?admin/parent_bulk_upload_error', 'refresh');
                }
...
function teacher($param1 = '', $param2 = '', $param3 = '') {
    ...
    if ($param1 == 'create') {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            redirect(base_url() . 'index.php?admin/teacher_information/', 'refresh');
        }
        ...
        if (count($rsTeacherData) > 0) {
            ...
            $this->session->set_flashdata('flash_message', "Unknown error arises to update the passcode");
        }
        $this->session->set_flashdata('flash_message', get_phrase('passcode_sent_successfully'));
        redirect(base_url() . 'index.php?admin/teacher_information/', 'refresh');
    } else if (($param1 == 'search') && ($param2 != '')) {
        $page_data['search_text'] = $param2;
    }
        ...
        if (empty($_FILES['userfile']['name'])) {
            ...
            if (in_array($_FILES['userfile']['type'], $allowed_types)) {
                ...
                if ($someRowError == FALSE) {
                    ...
                    $this->session->set_flashdata('flash_message_error', "Some rows are not uploaded,due to invalid data.");
                    redirect(base_url() . 'index.php?admin/subject_bulk_upload_error', 'refresh');
                }
            }//ends allowed type code
            else {
                $this->session->set_flashdata('flash_message_error', 'Sorry, File extension is not supported, Please enter data in Excel Spread Sheet!!');
...
function classes($param1 = '', $param2 = '') {
    ...
    if ($param1 == 'create') {
        ...
        $this->form_validation->set_rules('name_numeric', 'Class numeric name', 'numeric|trim|required|is_unique[class.name_numeric]');
        ...
        if (empty($_FILES['userfile']['name'])) {
            ...
            if (in_array($_FILES['userfile']['type'], $allowed_types)) {
                ...
                if ($someRowError == FALSE) {
                    ...
                    $this->session->set_flashdata('flash_message_error', "Some rows are not uploaded,due to invalid data.");
                    redirect(base_url() . 'index.php?admin/class_bulk_upload_error', 'refresh');
                }
            } else {
...
function delete_academic_syllabus($academic_syllabus_code, $class_id) {
    ...
    redirect(base_url() . 'index.php?admin/academic_syllabus/' . $class_id, 'refresh');
}
...
//                echo $count_room[0]['count_room'];exit;    
    ...
    if ($param1 == 'delete') {
        ...
        if (!empty($students)) {
            ...
            redirect(base_url() . 'index.php?admin/section/' . $param3, 'refresh');
        } else {
...
function get_exam_subject($param1 = '') {
    ...
    if ($param1 == 'save_subjects') {
        ...
        foreach ($subjects as $subject) {
            ...
            $data['start_datetime'] = $start_time[$count];
        ...
        redirect(base_url() . 'index.php?admin/exam_routine/');
    }
    ...
    //pre($page_data['exam_routine']); die();
    $this->load->model('Subject_model');
    /*$page_data['subjects'] = $this->Subject_model->get_subject_array(array("class_id" => $class_id, 'section_id' => $section_id));*/
...
function exam($param1 = '', $param2 = '', $param3 = '') {
    ...
    if ($param1 == 'import_excel') {
        ...
        list($num_cols, $num_rows) = $xlsx->dimension();
        $f = 0;
        $fielsdStringForAdmin = "Exam Name,Commencing Date,Academic Year,Comment,Evaluatiion Method";
        $fielsdString = "name,date,year,comment,grading";
        $fielsdStringMandotary = "name,date,year,comment,grading";
        $fielsdArr = explode(',', $fielsdString);
        $fielsdStringForAdminArr = explode(',', $fielsdStringForAdmin);
        $fielsdStringMandotaryArr = explode(',', $fielsdStringMandotary);
        $someRowError = FALSE;
        ...
        foreach ($xlsx->rows() as $r) {
            ...
            for ($i = 0; $i < $num_cols; $i++) {    // checking is filds is mandetory or not
                ...
                if (in_array($fielsdArr[$i], $fielsdStringMandotaryArr)) {
                    ...
                    if ($fielsdArr[$i] == 'name') {
                        $rsClass = $this->Exam_model->get_name($r[$i]);
            ...
            if ($error === FALSE) {
                $data['status'] = 1;
                //pre($data);die;
                $exam_id = $this->Exam_model->create_exam($data);
            } else {
                //pre($errorMsgArr);//die;
                $errorRowNo++;
                $errorExcelArr[] = $r;
        ...
        if ($someRowError == FALSE) {
            ...
            $this->session->set_flashdata('flash_message_error', "Some rows are not uploaded,due to invalid data.");
            redirect(base_url() . 'index.php?admin/exam_bulk_upload_error', 'refresh');
        }
    }
...
function online_exam($param1 = '', $param2 = '', $param3 = '') {
    ...
    if (isset($param1) && $param1 == 'create') {
        ...
        //exit;
        $this->session->set_flashdata('flash_message', get_phrase('exam_added'));
        redirect(base_url() . 'index.php?admin/online_exam/', 'refresh');
...
function add_question_online($param1 = '', $param2 = '', $param3 = '') {
    ...
    if (isset($param1) && $param1 == 'create') {
        ...
        redirect(base_url() . 'index.php?admin/add_subject_online_exam/' . $param1 . '/' . $param2 . '/' . $section_id, 'refresh');
        // print_r(json_encode($response_array));//exit;
    }
    ...
    if ($param1 == 'edit' && $param2 == 'do_update') {
        ...
        $this->session->set_flashdata('flash_message', get_phrase('data_updated'));
        redirect(base_url() . 'index.php?admin/add_subject_online_exam/' . $data['class_id'] . '/' . $data['subject_id'], 'refresh');
    } else if ($param1 == 'edit') {
        $page_data['edit_data'] = $this->Exam_model->get_online_data($param2);
    }
    if ($param1 == 'delete') {
        ...
        $this->session->set_flashdata('flash_message', get_phrase('data_deleted'));
        redirect(base_url() . 'index.php?admin/add_subject_online_exam/' . $param3, 'refresh');
    }
...
function add_subject_online_exam($param1 = '', $param2 = '', $param3 = '') {
    ...
    $question = $this->Question_model->get_details_ById($param1, $param3);
    $page_data['exam_id'] = $param2;
    $page_data['class_id'] = $param1;
...
function marks_manage_view($exam_id = '', $class_id = '', $section_id = '', $subject_id = '') {
    ...
    $page_data['running_year'] = $this->Setting_model->get_setting_record(array('type' => 'running_year'), 'description');
    /*$page_data['subjects'] = $this->Subject_model->get_data_by_cols('*', array('class_id' => $class_id, 'year' => $page_data['running_year']), 'result_type');*/
    if($grading_method == 3){
    ...
    if (isset($_POST['operation'])) {
        ...
        if ($page_data['exam_id'] > 0 && $page_data['class_id'] > 0) {
            /* echo $page_data['class_id']. '<br/>'. $page_data['section_id'].'<br/>'.$page_data['exam_id'];
              echo 'here'; exit; */
            redirect(base_url() . 'index.php?admin/tabulation_sheet/' . $page_data['class_id'] . '/' . $page_data['section_id'] . '/' . $page_data['exam_id'], 'refresh');
        } else {
...
function grade($param1 = '', $param2 = '') {
    ...
    if ($param1 == 'create') {
        ...
        $this->form_validation->set_rules('mark_from', 'mark_form', 'trim|required');
        $this->form_validation->set_rules('mark_upto', 'mark_upto', 'required|trim');
    ...
    if ($param1 == 'import_excel') {
        ...
        if ($someRowError == FALSE) {
            ...
            create_excel_file($file_name_with_path, $errorExcelArr, 'Exam Upload Data');
            $this->session->set_flashdata('flash_message_error', "Some rows are not uploaded,due to invalid data.");
            redirect(base_url() . 'index.php?admin/grade_bulk_upload_error', 'refresh');
        }
    }
   ...
   //$param1 -> Action like add, delete....$param2 ->value passed like id,name,... $param3 -> Exam Type   
    ...
    switch($param1){
        case 'add': $res = $this->Exam_model->group_add($data);
                    ...
                    redirect(base_url()."index.php?admin/exam_settings");
                    break;
...
//    } else{ 
//        $this->session->set_flashdata("flash_message_error", validation_errors());
//        //redirect(base_url()."/index.php?admin/exam_settings/igcse");
//    }
//    switch($param1){
//        case 'igcse': //
...
function cce_subjects($param1 = '', $param2 = '') {
    ...
    if ($param1 == 'do_update') {
        ...
        /*redirect(base_url() . 'index.php?admin/exam_settings/', 'refresh');*/
        redirect($_SERVER['HTTP_REFERER']);
    }
...
function attendance_update($class_id = '', $section_id = '', $timestamp = '') {
    ...
    if (empty($locationData)) {
        ...
        rediect(base_url() . 'index.php?admin/attendance_report');
    } else {
        $location = $this->globalSettingsLocation;
    }
...
function attendance_report_view($class_id = '', $section_id = '', $month = '') {
    ...
    $this->load->model('Student_model');
    $this->load->model("Holiday_model");
    $holidays=$this->Holiday_model->get_holiday_list();
    $holiday_dates=array();
    ...
    foreach ($page_data['students'] as $k => $v) {
        ...
        for ($i = 1; $i <= $page_data['days']; $i++) {
           ...
           //pre($page_data['students'][$k]['atten'][$p]);
            $p++;
        }
...
//        $data['route_fare'] = $this->input->post('route_fare');        
    ...
    if ($param1 == 'import_excel') {
        ...
        list($num_cols, $num_rows) = $xlsx->dimension();
        $f = 0;
        $fielsdStringForAdmin = "Route Name,Route From,Route To,Number Of Stops,Route Fare Name";
        $fielsdString = "route_id,route_from,route_to,no_of_stops,route_fare";
        $fielsdStringMandotary = $fielsdString;
        $fielsdArr = explode(',', $fielsdString);
        $fielsdStringForAdminArr = explode(',', $fielsdStringForAdmin);
        $fielsdStringMandotaryArr = explode(',', $fielsdStringMandotary);
            ...
            for ($i = 0; $i < $num_cols; $i++) {    // checking is filds is mandetory or not
                ...
                if (in_array($fielsdArr[$i], $fielsdStringMandotaryArr)) {
                    ...
                    if ($fielsdArr[$i] == 'route_fare') {
                        ...
                        $routeFareArr = $this->Transport->get_rout_fare(6, $this->globalSettingsRunningYear, "'" . trim($r[$i]) . "'");
        ...
        if ($someRowError == FALSE) {
            ...
            create_excel_file($file_name_with_path, $errorExcelArr, 'Exam Upload Data');
            $this->session->set_flashdata('flash_message_error', "Some rows are not uploaded,due to invalid data.");
            redirect(base_url() . 'index.php?admin/routes_bulk_upload_error', 'refresh');
        }
    }
...
function noticeboard($param1 = '', $param2 = '', $param3 = '') {
    ...
    if ($param1 == 'create') {
        ...
        if ($this->form_validation->run() == TRUE) {
            ...
            send_school_notification('event_notice', $message, '', '', $user_details, $data['class_id']);
...
function message($param1 = 'message_home', $param2 = '', $param3 = '') {
    ...
    if ($param1 == 'message_read') {
        ...
        $page_data['messages'] = $this->Message_model->get_data_by_cols('*', array('message_thread_code' => $param2,'message_status!='=>'admin_deleted'), 'result_array');
        $parent = array();
        $parent_all = array();
        $i = 0;
        $NewArray = array();
...
//            $img_user[]['image'] = $this->crud_model->get_image_url($sender_account_type, $sender_id);
            ...
            if ($sender_account_type == 'parent') {
                ...
                if($model_name  ==  "teacher_model")
                {
                  $parent = $this->$model_name->get_data_by_cols("name,teacher_image", array($sender_account_type . '_id' => $sender_id), 'result_array');
                    ...
                    if (!empty($parent)) {
                        $parent_all[]['name'] = $parent[0]['name'];
                        $image[]['image'] = "admin_image/".$parent[0]["image"];
                    } else {
                        $parent_all[]['name'] = "";
                        $image[]['image']   =   "user.png";
                    }
    ...
    if ($param1 == 'message_new') {
        ...
        $all_participent['teacher'] = $this->Teacher_model->get_data_by_cols('*', array(), 'result_array', array('name' => 'asc'));

        $all_participent['student'] = $this->Student_model->get_data_by_cols('*', array(), 'result_array', array('name' => 'asc'));

        $all_participent['parent'] = $this->Parent_model->get_data_by_cols('*', array(), 'result_array', array('father_name' => 'asc'));
    ...
    if ($param1 == 'delete') {
        ...
        if ($delete_message) {
            ...
            redirect(base_url() . 'index.php?admin/message/message_read/' . $thread_code, 'refresh');
        } else {
    ...
    foreach ($page_data['message_threads'] as $k => $v) {
        ...
        if ($user_to_show_type == "student") {
            $page_data['message_threads'][$k]['unread_message_number_student'] = $this->crud_model->count_unread_message_of_thread($v['message_thread_code']);
        ...
        if ($user_to_show_type == "teacher") {
            $page_data['message_threads'][$k]['unread_message_number_teacher'] = $this->crud_model->count_unread_message_of_thread($v['message_thread_code']);
        ...
        if ($user_to_show_type == "parent") {
            $page_data['message_threads'][$k]['unread_message_number_parent'] = $this->crud_model->count_unread_message_of_thread($v['message_thread_code']);
...
function system_settings($param1 = '', $param2 = '', $param3 = '') {
    ...
    if ($param1 == 'do_update') {
        ...
        $data['description'] = $this->input->post('google_drive_mail_id');
...
function manage_profile($param1 = '', $param2 = '', $param3 = '') {
    ...
    if ($param1 == 'change_password') {
        ...
        $current_password = $this->Admin_model->get_data_by_cols('password', array('admin_id' => $this->session->userdata('admin_id')), 'result_array');
...
function device($param1 = '', $param2 = '', $param3 = '') {
    ...
    if ($param1 == 'create') {
        ...
        redirect(base_url() . 'index.php?admin/manage_device/', 'refresh');
    }
    if ($param1 == 'do_update') {
        $data['Name'] = $this->input->post('Name');
        $data['Imei'] = $this->input->post('Imei');
...
function bus($param1 = '', $param2 = '') {
    ...
    if ($param1 == 'create') {
        ...
        if ($this->form_validation->run() == TRUE) {
            ...
            $this->session->set_flashdata('flash_message', get_phrase('data_added_successfully'));
            redirect(base_url() . 'index.php?admin/bus/', 'refresh');
        }else {
...
//            $this->load->view('backend/index', $page_data);
    ...
    if ($param1 == 'import_excel') {
        ...
        if ($someRowError == FALSE) {
            ...
            create_excel_file($file_name_with_path, $errorExcelArr, 'Exam Upload Data');
            $this->session->set_flashdata('flash_message_error', "Some rows are not uploaded,due to invalid data.");
            redirect(base_url() . 'index.php?admin/bus_details_bulk_upload_error', 'refresh');
        }
    }
    if ($param1 == '') {
//        $page_data['buses'] = $this->Bus_driver_modal->get_bus_with_route1();
...
function bus_admin($param1 = '', $param2 = '') {
    ...
    if ($param1 == 'create') {
        ...
        $this->session->set_flashdata('flash_message', get_phrase('data_added_successfully'));
        //$this->Email_model->account_opening_email('parent', $data['email'], $this->input->post('password')); //SEND EMAIL ACCOUNT OPENING EMAIL
        redirect(base_url() . 'index.php?admin/bus_admin/', 'refresh');
    }
    if ($param1 == 'edit') {
        $data['name'] = $this->input->post('name');
        $data['email'] = $this->input->post('email');
...
function bus_driver($param1 = '', $param2 = '', $param3 = '') {
    ...
    if ($param1 == 'create') {
        ...
        if ($this->form_validation->run() == FALSE) {
            $this->session->set_flashdata('flash_message_error', validation_errors());
            redirect(base_url() . 'index.php?admin/bus_driver/');
        } else {
            $data['name'] = $this->input->post('name');
            $data['email'] = $this->input->post('email');
            //$data['password']   =   sha1($this->input->post('password'));
            ...
            $this->session->set_flashdata('flash_message', get_phrase('data_added_successfully'));
            //$this->Email_model->account_opening_email('parent', $data['email'], $this->input->post('password')); //SEND EMAIL ACCOUNT OPENING EMAIL
            redirect(base_url() . 'index.php?admin/bus_driver/', 'refresh');
        }
    } else if ($param1 == 'edit') {
        ...
        if (empty($_FILES['userfile']['name'])) {
            ...
            if (in_array($_FILES['userfile']['type'], $allowed_types)) {
                    ...
                    for ($i = 0; $i < $num_cols; $i++) {    // checking is filds is mandetory or not
                        ...
                        if (in_array($fielsdArr[$i], $fielsdStringMandotaryArr)) {
                            ...
                            if (trim($r[$i]) == "") {
                                ...
                                if ($fielsdArr[$i] == 'email') {
                                    //echo 'Meera';die();
                                    $rsEmailPhoneUnique = $this->Bus_driver_model->get_data_by_cols('bus_driver_id', array('email' => trim($r[$i])));
                                    $validPhoneEmailCheck = $this->checkValidPhoneEmail(trim($r[$i]), 'email');
                                } elseif ($fielsdArr[$i] == 'phono') {
                                    $rsEmailPhoneUnique = $this->Bus_driver_model->get_data_by_cols('bus_driver_id', array('phone' => trim($r[$i])));
                                    $validPhoneEmailCheck = $this->checkValidPhoneEmail(trim($r[$i]), 'phone');
                                }
                                ...
                                if ($fielsdArr[$i] == 'bus_id') {
                                    $routeArr = $this->Bus_model->get_name($r[$i]);
                                    if (empty($routeArr)) {
                                        $error = TRUE;
                                        $errorMsgArr[] = $fielsdStringForAdminArr[$i] . " is not exists at row no -" . $errorRowNo;
                                    } else {
                ...
                if ($someRowError == FALSE) {
                    ...
                    $this->session->set_flashdata('flash_message_error', "Some rows are not uploaded,due to invalid data.");
                    redirect(base_url() . 'index.php?admin/bus_driver_bulk_upload_error', 'refresh');
                }
            }//ends allowed type code
            else {
                $this->session->set_flashdata('flash_message_error', 'Sorry, File extension is not supported, Please enter data in Excel Spread Sheet!!');
...
function marks_bulk_upload($param = '') {
    ...
    if ($param = 'import_excel') {
        ...
        if ($someRowError == FALSE) {
            ...
            create_excel_file($file_name_with_path, $errorExcelArr, 'Exam Upload Data');
            $this->session->set_flashdata('flash_message_error', "Some rows are not uploaded,due to invalid data.");
            redirect(base_url() . 'index.php?admin/mark_bulk_upload_error', 'refresh');
        }
    }
}
...
function send_all_notification_by_global_server($deviceLocation, $cell_phone, $dt, $gender, $name, $email, $parent_id) {
    ...
    $url = "http://52.29.203.220/School/index.php/?admin/send_all_notification_to_parrent_about_student/";
    /* $ch = curl_init($url);
...
function automatic_time_table() {
    $page_data = $this->get_page_data_var();
    $page_data['page_title'] = get_phrase('automatic_time_table');
    $page_data['page_name'] = 'automatic_time_table';
    $page_data['total_notif_num'] = $this->get_no_of_notication();
    $this->load->view('backend/index', $page_data);
}
...
function ptm_settings($param1 = '', $param2 = '') {
    ...
    $data['class_id'] = $this->input->post('class_id');
    $data['section_id'] = $this->input->post('section_id');
    $data['exam_id'] = $this->input->post('exam_holder');
    $data['meeting_date'] = date('Y-m-d', strtotime($this->input->post('date_select')));
    ...
    if ($formSubmit == 'submit_date') {
        ...
        redirect(base_url() . 'index.php?admin/ptm_settings', 'refresh');
    }
    if ($param1 == 'delete') {
    ...
    foreach ($ptm_settings as $k => $value) {
        ...
        $ptm_settings[$k]['exam_name'] = $this->Exam_model->get_name_by_id($value['exam_id']);
...
function inventory_category($param1 = '', $param2 = '', $param3 = '') {
    ...
    if ($param1 == 'create') {
        if ($_POST) {
            ...
            redirect(base_url() . 'index.php?admin/inventory_category/', 'refresh');
        }
    }
    if ($param1 == 'delete') {
...
function seller_master($param1 = '', $param2 = '', $param3 = '') {
    ...
    if ($param1 == 'delete') {
        ...
        redirect(base_url() . 'index.php?admin/seller_master/', 'refresh');
    }
    if ($param1 == 'do_update') {
        ...
        if ($this->form_validation->run() == FALSE) {
            $id = $param2;
            $this->session->set_flashdata('flash_message_error', validation_errors());
            redirect(base_url() . 'index.php?admin/seller_edit/' . $id);
        } else {
            if ($_POST) {
...
function seller_add() {
    ...
    if ($this->form_validation->run() == TRUE) {
        if ($_POST) {
            ...
            $data['seller_contact_person'] = $_POST['contactName'];
            $data['seller_address'] = $_POST['sellerAddress'];
            $sql = $this->Seller_model->add($data);
            if ($sql === TRUE) {
...
function product($param1 = '', $param2 = '', $param3 = '') {
    ...
    if ($param1 == 'create') {
        ...
        for ($i = 1; $i <= $no_of_products; $i++) {
            ...
            $data['Quantity'] = 1;
            $data['categories_id'] = $this->input->post('category'); //echo $categories_name; exit;
        ...
        redirect(base_url() . 'index.php?admin/manage_product/', 'refresh');
    }
...
function parent_add($param1 = '') {
    ...
    $page_data['total_notif_num'] = $this->get_no_of_notication();
    if ($param1 == 'create') {
        $this->form_validation->set_rules('fname', 'Father First Name', 'trim|required');
//        $this->form_validation->set_rules('flname', 'Father  Last Name', 'trim|required');
        $this->form_validation->set_rules('mname', 'Mother First Name', 'trim|required');
//        $this->form_validation->set_rules('mlname', 'Mother Last Name', 'trim|required');
...
//        $this->form_validation->set_rules('email', 'Email', 'trim|required|valid_email|is_unique[parent.email]');
        $this->form_validation->set_rules('emergency_contact', 'Emergency Contact Number', 'trim|required|numeric|max_length[10]');
        //$passcode="spa".mt_rand(10000000, 99999999);
        if ($this->form_validation->run() == TRUE) {
            $data['father_name'] = $this->input->post('fname');
            $data['father_mname'] = $this->input->post('fmname');
...
function approve_student() {
    ...
    $this->session->set_flashdata('flash_message', get_phrase('data_updated'));
    redirect(base_url() . 'index.php?admin/student_applied', 'refresh');
}
...
function admission_enquiry() {
    ...
    $this->load->model("Guardian_model");
    $page_data = $this->get_page_data_var();
    $this->form_validation->set_rules('mobile_number', 'Mobile Number', 'required|numeric|max_length[11]');
    ...
    $this->form_validation->set_rules('class', 'Class', 'required|callback_check_class_student_capacity');
    $this->form_validation->set_rules('previous_class', 'Previous Class', 'required');
    if ($this->input->post('previous_class') != '-10') {
    ...
    $this->form_validation->set_rules('region', 'State', 'required');
    $this->form_validation->set_rules('zip_code', 'Zip Code', 'required');
    $this->form_validation->set_rules('country', 'Country', 'required');
    //$this->form_validation->set_rules('guardian_emergency_number', 'Emergency Contact', 'required|numeric|max_length[12]');
...
//        $admit_data['parent_lname'] = $this->input->post('parent_lname');
        ...
        /* $running_year = $this->Setting_model->get_setting_record(array('type' => 'running_year'), 'description');
          ...
          redirect(base_url() . 'index.php?admin/admission_enquiry', 'refresh');
        ...
        if ($guardian_id) {
            ...
            redirect(base_url() . 'index.php?admin/admission_enquiry/');
        } else {
        ...
        if (!empty($sub_applicants)) {
            ...
            foreach ($sub_applicants as $student) {
                ...
                if($form_no_record){
                    ...
                    redirect(base_url() . 'index.php?admin/student_enquired_view/', 'refresh');
                }   
...
function inventory_allotment($param1 = '', $param2 = '', $param3 = '') {
    ...
    if ($param1 == 'create') {
        ...
        if ($this->form_validation->run() == TRUE) {
            ...
            $teacherID = $this->Teacher_model->get_teacher_by_name($teacher_name);
            if ($teacherID != '') {
                $data['teacher_id'] = $teacherID->teacher_id;
            } else {
                $data['teacher_id'] = '';
            }
            ...
            $this->session->set_flashdata('flash_message_error', validation_errors());
            redirect(base_url() . 'index.php?admin/inventory_add_allotment/' . $param2, 'refresh');
        }
    }
}
...
function view_admission_form($param1 = '') {
    ...
    if ($form_name == 'admit_student1') {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            redirect(base_url() . 'index.php?admin/view_admission_form/' . $param1);
        } else {
            $parentIdArray = array();
            ...
            if (empty($parentIdArray)) {
                ...
                redirect(base_url() . 'index.php?admin/student_enquired_view/');
            }
            ...
            $parent_id = $parentIdArray->parent_id;
...
function regenerate_passcode_parent($param1 = '') {
    ...
    $this->Parent_model->update_parent(array('passcode' => $passcode, 'password' => sha1($passcode)), $dataArray);
    $parentDataArr = $this->Parent_model->get_data_by_cols('cell_phone,father_name,passcode,email', $dataArray);
    $parent = $parentDataArr[0];

    $message = "Welcome Mr " . $parent->father_name . " your passcode for app is " . $passcode . "   download app here https://play.google.com/store/apps/details?id=" . $this->globalSettingsAppPackageName . "&hl=en";
...
function notification_dashboard() {
    ...
    if(isset($update_result)) {
        ...
        redirect(base_url() . 'index.php?admin/notification_dashboard' , 'refresh');
    }
...
function fix_clinical_appointment($param1 = '') {
    ...
    if ($formSubmit == 'save_details') {
        ...
        if ($this->Medical_events_model->save_appointment($apppointment)) {
            ...
            redirect(base_url() . 'index.php?admin/clinical_histroy/' . $param1, 'refresh');
        } else {
            $this->session->set_flashdata('flash_message_error', get_phrase('appointment_not_fixed_succesfully!!'));
            redirect(base_url() . 'index.php?admin/fix_clinical_appointment/' . $param1, 'refresh');
        }
    }
...
function connect_user_to_chat_group($group_id = "", $param1 = "", $action_id = "") {
    ...
    if ($param1 == 'create') {
        ...
        redirect(base_url() . 'index.php?admin/connect_user_to_chat_group/' . $group_id, 'refresh');
    }
    ...
    if ($formname == 'add_discussion') {
        if ($param1 == 'create') {
            ...
            if ($this->Discussion_model->insert_discussion($data)) {
                ...
                redirect(base_url() . 'index.php?admin/discussion_forum/', 'refresh');
            } else {
...
function documents($param1 = '') {
    ...
    $page_data['subfiles'] = $this->S3_model->get_file($page_data['files'][0], $instance . '/student/' . $param1 . '/');
...
//    echo $file_path; die;
    ...
    redirect(base_url() . 'index.php?admin/documents/' . $param3, 'refresh');
}
...
function upload_document($param1 = '') {
    ...
    if (!$this->upload->do_upload('userfile')) {
        ...
        redirect(base_url() . 'index.php?admin/documents/' . $param1, 'refresh');
    } else {
...
function event_management($param1 = '') {
    ...
    if ($param1 == 'create') {
        ...
        if ($this->form_validation->run() == TRUE) {
            ...
            redirect(base_url() . 'index.php?admin/event_management/', 'refresh');
        } else {
            $page_data['page_name'] = 'noticeboard';
            $page_data['page_title'] = get_phrase('manage_noticeboard');
...
function event($param1 = '', $param2 = '') {
    ...
    if ($param1 == 'create') {
        ...
        //$data['start'] = $this->input->post('startTime'); //date("Y-m-d",$timestamp);
...
public function map_students_id_after_referesh($class_id = '') {
    ...
    redirect(base_url() . 'index.php?admin/map_students_id/' . $class_id, 'refresh');
}
...
public function payment_config($param1 = '') {
    ...
    if ($this->form_validation->run() == TRUE) {
        ...
        if ($this->Payment_gateway_model->save_payment_options($payment_options)) {
            ...
            redirect(base_url() . 'index.php?admin/view_payment_details');
        } else {
            $this->session->set_flashdata('flash_message_error', get_phrase('details_not_added!!'));
            redirect(base_url() . 'index.php?admin/payment_config');
        }
    } else {
        $this->session->set_flashdata('flash_message_error', validation_errors());
...
public function view_payment_details($param1 = '', $param2 = '') {
    ...
    if ($param1 == 'delete') {
        ...
        if ($this->Payment_gateway_model->delete_payment_options($condition)) {
            $this->session->set_flashdata('flash_message', get_phrase('data_deleted'));
            redirect(base_url() . 'index.php?admin/view_payment_details', 'refresh');
        } else {
...
function add_warden() {
    ...
    if ($this->form_validation->run() == TRUE) {
        ...
        if ($sql == true) {
            $this->session->set_flashdata('flash_message', get_phrase('data_added_successfully'));
            redirect(base_url() . 'index.php?admin/manage_hostel_warden', 'refresh');
        } else {
            $this->session->set_flashdata('flash_message_error', get_phrase('something_went_wrong'));
            redirect(base_url() . 'index.php?admin/add_warden', 'refresh');
        }
    }
...
function add_hostel() {
    ...
    if ($this->form_validation->run() == TRUE) {
        ...
        $this->session->set_flashdata('flash_message', get_phrase('data_added_successfully'));
        redirect(base_url() . 'index.php?admin/manage_hostel', 'refresh');
    }
    $this->load->model("Hostel_warden_model");
    $page_data['total_notif_num'] = $this->get_no_of_notication();
...
function add_hostel_room() {
    ...
    if ($this->form_validation->run() == TRUE) {
        ...
        $this->fee_fi_model->add($data2);
        $this->session->set_flashdata('flash_message', get_phrase('data_added_successfully'));
        redirect(base_url() . 'index.php?admin/manage_hostel_room', 'refresh');
    }
    $this->load->model("Dormitory_model");
    $page_data['charges'] = $charges;
    $page_data['total_notif_num'] = $this->get_no_of_notication();
...
function hostel_registration() {
    ...
    if ($this->form_validation->run() == TRUE) {
        ...
        $this->session->set_flashdata('flash_message', get_phrase('data_added_successfully'));
        redirect(base_url() . 'index.php?admin/manage_allocation', 'refresh');
    }
    $this->load->model("Class_model");
    $class_array = $this->Class_model->get_class_array();
    $page_data['classes'] = $class_array;
...
function get_hostel_name($type = "",$student_id = "") {
    $student_gender =   $this->Student_model->get_data_by_cols('sex',array('student_id'=> $student_id),'result_array');
...
function get_details($room_no = "") {
    ...
    $name = $new;
    foreach ($name as $row) {
        echo '<span>' . '<ul class="list-unstyled badge-danger badge_style_over list-inline p-10" >' .
...
function hostel_transfer($student_id = "", $exsiting_room_no = "", $hostel_reg_id = "") {
    ...
    if ($this->form_validation->run() == TRUE) {
        ...
        $this->session->set_flashdata('flash_message_error', validation_errors());
        //redirect(base_url() . 'index.php?admin/hostel_transfer');
...
function manage_hostel_warden($param1 = "", $warden_id = "") {
    ...
    $this->load->model('Hostel_warden_model');
    $page_data['total_notif_num'] = $this->get_no_of_notication();
    $page_data['warden_details'] = $this->Hostel_warden_model->get_data_by_cols('*', array('status' => "Active"), 'result_array', array('created_date' => 'ORDER BY', 'created_date' => 'DESC'));
...
public function send_sms($phone_number, $message) {
    ...
    $url = "http://52.29.203.220/School/index.php/?admin/send_common_sms/"; // url for sms configured
...
public function campus_updates_management($param1 = '', $param2 = '') {
    ...
    if ($param1 == 'create') {
        ...
        if ($this->Notification_model->save_notifications($campus_update)) {
            ...
            redirect(base_url() . 'index.php?admin/campus_updates_management');
        } else {
    ...
    if ($param1 == 'delete') {
        ...
        if ($this->Notification_model->delete_updates($condition)) {
            $this->session->set_flashdata('flash_message', get_phrase('data_deleted'));
            redirect(base_url() . 'index.php?admin/campus_updates_management', 'refresh');
        } else {
            $this->session->set_flashdata('flash_message_error', get_phrase('data_not_deleted'));
            redirect(base_url() . 'index.php?admin/campus_updates_management', 'refresh');
        }
...
function print_transfer_certificate($param1 = '', $param2 = '', $param3 = '') {
    if ($this->session->userdata('school_admin_login') != 1)
        redirect(base_url(), 'refresh');
...
//         pre($page_data['parent_name']);
//        echo $parent_name['mother_name']; die;
...
//    $this->form_validation->set_rules('end_date', 'End Date', 'trim|required');
    if ($this->form_validation->run() == TRUE) {
        ...
        if ($count[0]['count'] <= 0) { 
            ...
            $this->session->set_flashdata('flash_message', get_phrase('data_added_successfully'));
            redirect(base_url() . 'index.php?admin/manage_student_bus', 'refresh');
        } else {
            $this->session->set_flashdata('flash_message_error', get_phrase('this_student_is_already_using_bus_facility'));
            redirect(base_url() . 'index.php?admin/student_bus_allocation', 'refresh');
        }
    } else {
        $this->session->set_flashdata('flash_message_error', validation_errors());
        redirect(base_url() . 'index.php?admin/student_bus_allocation', 'refresh');
...
function bus_details($param1 = "") {
    ...
    $page_data['page_title'] = get_phrase('vehicle_service_&_maintenance_details');
    ...
    if ($param1 == 'create') {
        ...
        if ($this->form_validation->run() == TRUE) {
            ...
            $data['driver_id'] = $this->input->post('bus_driver_id');
            ...
            $data['status'] = "Active";
            if ((!empty($data['route_id'])) && (!empty($data['driver_id']))) {
                ...
                redirect(base_url() . 'index.php?admin/bus_details', 'refresh');
            }
        } else {
            $this->session->set_flashdata('flash_message_error', validation_errors());
            redirect(base_url() . 'index.php?admin/bus_details', 'refresh');
...
function route_bus_stop($param1 = '', $param2 = '', $param3 = '') {
    ...
    if ($param1 == 'create') {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $this->Fee_fi_model->add($data2);
            $this->session->set_flashdata('flash_message', get_phrase('data_added_successfully'));
            redirect(base_url() . 'index.php?admin/route_bus_stop', 'refresh');
        }
    }
    if ($param1 == 'edit') {
        $this->form_validation->set_rules('route_id', 'Route Name', 'trim|required');
...
function fee_structure($param1 = '', $param2 = '', $param3 = '') {
    ...
    if ($param1 == 'create') {
        ...
        if ($this->upload->do_upload('fee_structure')) {
            ...
            redirect(base_url() . 'index.php?admin/fee_structure', 'refresh');
        } else {
...
function admission_settings_update() {
    ...
    if ($this->form_validation->run() == false) {
        ...
        redirect(base_url() . 'index.php?admin/admission_settings', 'refresh');
    } else {
...
function send_sms_for_low_attendance($student_id = '', $percentage = '') {
    ...
    $message = 'Your child' . ' ' . $student_name . 'has attendence percentage of' . $percentage . '% this month';
    echo send_school_notification('stud_absence', $message, array($receiver_phone));
}
...
function update_study_material($param1 = '') {
    ...
    if (in_array($_FILES['file_name']['type'], $allowed_types)) {
        ...
        redirect(base_url() . 'index.php?admin/study_material/' . $class_id, 'refresh');
    } else {
...
//CREATE NEEW CERTIFICATE by Beant Kaur
function create_ceritificate($student_id) {
    if ($this->session->userdata('school_admin_login') != 1)
        redirect(base_url(), 'refresh');
    $page_data = $this->get_page_data_var();
...
function experience_certificate($param1) {
    $rsTeacherData = $this->Teacher_model->get_data_by_cols('name,middle_name,last_name,email', array('teacher_id' => $param1));
    $page_data = $this->get_page_data_var();
...
function internship_certificate($param1) {
    $rsTeacherData = $this->Teacher_model->get_data_by_cols('name,middle_name,last_name,email', array('teacher_id' => $param1));
    $page_data = $this->get_page_data_var();
    $page_data['teacher_data'] = $rsTeacherData;
    $page_data['teacher_id'] = $param1;
...
function student_bus($param1, $student_bus_id) {
    ...
    if ($param1 == 'edit') {
        ...
        if ($this->form_validation->run() == FALSE) {
            $this->session->set_flashdata('flash_message_error', validation_errors());
            redirect(base_url() . 'index.php?admin/manage_student_bus');
        } else {
            $data['route_id'] = $this->input->post('route');
            $data['bus_stop_id'] = $this->input->post('bustop_id');
            $data['bus_id'] = $this->input->post('bus');
...
function ibo_program($param1 = '', $param2 = '') {
    ...
    if ($param1 == 'add') {
        ...
        $page_data['program_name'] = $program_name;
        $this->Class_model->ibo_add_program($page_data);
...
function ibo_exam_connect($param1 = '', $param2 = '') {
    ...
    function marks_update_ibo($exam_id = '', $class_id = '', $section_id = '', $subject_id = '') {
       ...
       //pre($_POST);// die;
        ...
        //foreach ($marks_of_students as $row) {
        for($k=0;$k<2;$k++) {
            //$obtained_marks[] = $this->input->post('d_marks_obtained_' . $row['mark_id']);
            $obtained_marks[] = $this->input->post('marks_obtained_'.$k);
...
//pre( $assessment);
                ...
                if($count_get_data > 0){
                    $this->Mark_model->ibo_update_marks(array('exam_id'=>$exam_id,'class_id'=>$class_id,'section_id'=>$section_id,'subject_id'=>$subject_id,'assessment_id'=>$assessment[$i],'student_uid'=>$student[$i]), array('mark_obtained' => $mark_obtained1[$i], 'comment' => $comment, 'mark_total' => $maximum_marks));
                }else{ //echo $assessment[$i]; die;
                    $this->db->insert('ibo_marks', array('exam_id'=>$exam_id,'class_id'=>$class_id,'section_id'=>$section_id,'subject_id'=>$subject_id,'assessment_id'=>$assessment[$i],'student_uid'=>$student[$i],'mark_obtained' => $mark_obtained1[$i], 'comment' => $comment, 'mark_total' => $maximum_marks));
...
function mess_management($param1 = "") {
    ...
    if ($param1 == "create") {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $this->session->set_flashdata('flash_message', get_phrase('data_added_successfully'));
            redirect(base_url() . 'index.php?admin/mess_management/', 'refresh');
        }
    }
    ...
    if ($param1 == "create") {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            if ($count_day[0]['count_day'] == 0) {
                ...
                $this->session->set_flashdata('flash_message', get_phrase('data_added_successfully'));
                redirect(base_url() . 'index.php?admin/mess_timetable/' . $data['mess_management_id'], 'refresh');
            } else {
                ...
                redirect(base_url() . 'index.php?admin/add_mess_time_table/', 'refresh');
            }
        }
    }
    if ($param1 == "edit") {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            redirect(base_url() . 'index.php?admin/mess_timetable/' . $mess_id, 'refresh');
        } else {
            $data['food'] = $this->input->post('food');
...
/*CCE Grading Method*/
    ...
    function csa_marks_selector()
    {
        ...
        if ($query->num_rows())
        {
            ...
            foreach ($students as $row)
            {
                $chkExist = $this->db->get_where('cce_coscholatic_grades', array(
                        'class_id' => $data['class_id'],'student_id' => $row['student_id'],'csa_id' => $data['csa_id'],'year' => $data['year'], 'term' => $data['term']
                    ))->result_array();
                if(count($chkExist) == 0) {
                    $data['student_id'] = $row['student_id'];
                    //pre($data); die();
                    $this->db->insert('cce_coscholatic_grades', $data);
                }
                
            }
        }
        redirect(base_url() . 'index.php?admin/csa_marks_manage_view/'  . $data['class_id'] . '/' . $data['csa_id']. '/' . $data['term'], 'refresh');
    }
    ...
    function csa_marks_manage_view($class_id = '', $csa_id = '', $term = '')
    {
        ...
        foreach($page_data['marks_of_students'] as $k => $v){
            ...
            $page_data['marks_of_students'][$k]['result'] = $this->Enroll_model->get_data_generic_fun("*", $data_arr, "result");
            $data_ar = array('student_id' => $row['student_id']);
            $page_data['marks_of_students'][$k]['result1'] = $this->Student_model->get_data_generic_fun("*", $data_ar, "result");
        }
    ...
    function cce_marksheet_selector() {
        ...
        redirect(base_url() . 'index.php?admin/cce_marksheet_view/' . $data['class_id'] . '/' . $data['student_id'] , 'refresh');
    }
    ...
    function cce_marksheet_view($class_id = '', $student_id='') {
        ...
        $this->load->model('Subject_model');
        $page_data['subjects'] = $this->Subject_model->marks_get_subject($class_id);
        ...
        /*Student Details*/
        $student       =   get_data_generic_fun('student','*',array('student_id'=>$student_id),'result_arr');
        $parent_names  =   array();
            foreach ($student as $row){
        ...
        foreach($year as $row1){
            $page_data['student_info']  =   get_data_generic_fun('enroll','*',array('student_id' =>$student_id,'year'=>$row1['description']),'result_arr');
        }
        $this->load->view('backend/index', $page_data);
    }
    /*CCE Grading Method*/
    ...
    function mark_bulk_upload(){
            ...
            if(!empty($class_data)){
                ...
                if($exam_data_str!=""){
                    ...
                    if(count($exam_data_str_arr)>1){
                        ...
                        //die;
                        $allSubjectArr=$allWorksheetDataArr[$firstKey][1];
                                                ...
                                                foreach ($tempStudentMarkArr AS $subjectIndex => $mark){
                                                    ...
                                                    if($subjectIndex % 2 == 0){
                                                        ...
                                                        if($mark==""){
                                                            ...
                                                            $errorMsgArr[]="Unable to update mark for worksheet(".$sheetName.") for subject(".$subjectIdArr[$mark_index]['subejct_name'].") for (".$studentNameIdArr[0].")";
                                                            generate_log(current($errorMsgArr), $log_file_name);
                                                        }else{
                                                        ...
                                                        if($subjectIdArr[$mark_index]['subject_id']=='invalid_subject_id'){
                                                            ...
                                                            if($subjectMark!=""){
                                                                if($subjectMark>$subjectIdArr[$mark_index]['maximum_mark']){
                                                                    /// error process
                                                                    $someRowError=true;//pre("at 13105");
                                                                    $errorMsgArr[]="Unable to update mark for worksheet(".$sheetName.") for student (".$studentNameIdArr[0].") for subject(".$subjectIdArr[$mark_index]['subejct_name']." Mark) greater than the maximum mark";
                                                                    generate_log(current($errorMsgArr), $log_file_name);
                                                                }else{
        ...
        if ($someRowError == FALSE) {
            ...
            die("0");
            //$this->session->set_flashdata('flash_message', get_phrase('mark_details_added'));
            //redirect(base_url() . 'index.php?admin/view_marks_bulk_upload/', 'refresh');
        } else {
            //pre($errorMsgArr);die;
            generate_log(json_encode($errorMsgArr), 'mark_bulk_upload_error_details.log', TRUE);
            /*$file_name_with_path = 'uploads/mark_bulk_upload_error_details_for_excel_file.xlsx';
            ...
            die("3");
            //$this->session->set_flashdata('flash_message_error', "Some rows are not uploaded,due to invalid data.");
            //redirect(base_url() . 'index.php?admin/mark_bulk_upload_error' . $this->input->post('class_id'), 'refresh');
        }
    }
    ...
    public function online_polls( $action = '' , $poll_id = '') {
        ...
        if($action == 'delete') {
            ...
            if($delete_result) {
                ...
                redirect(base_url() . 'index.php?admin/online_polls/', 'refresh');
            } else {
    ...
    function database_data_backup_list(){
        ...
        foreach (new DirectoryIterator('database_data_backups/') as $fileInfo) {
            //pre($fileInfo);
            $fileData=array();
            if($fileInfo->isDot()) {continue;}else{$i++;}
    ...
    function create_database_data_backup(){
        ...
        redirect(base_url() . 'index.php?admin/database_data_backup_list/', 'refresh');
    }
    ...
    public function home_works( $action = '' , $home_work_id = '' ) {
        ...
        if($action == 'create') {
            ...
            redirect(base_url() . 'index.php?admin/home_works/', 'refresh');
        } else if($action == 'delete') {
            if($home_work_id)
                $delete =   $this->Homeworks_model->update_data(array('homework_typeid'=>(int)$home_work_id),array('status'=>3),'home_work_types');
            if(isset($delete)){
    ...
    function deactivate_holiday($param1="")
    {
        ...
        $this->load->model("Holiday_model");
        $this->Holiday_model->deactivate_holiday($id);
        $page_data['holidays']=$this->Holiday_model->get_holiday_list();
        $page_data['page_name']         =   'holiday_settings';
        $page_data['page_title']        =   get_phrase('holiday_settings');
        $this->load->view('backend/index', $page_data);
    ...
    function activate_holiday($param1="")
    {
        ...
        $id=$this->input->post("id");
        $this->load->model("Holiday_model");
        $this->Holiday_model->activate_holiday($id);
        $page_data['holidays']=$this->Holiday_model->get_holiday_list();
        $page_data['page_name']         =   'holiday_settings';
        $page_data['page_title']        =   get_phrase('holiday_settings');
        $this->load->view('backend/index', $page_data);
