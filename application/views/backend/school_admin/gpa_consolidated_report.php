    ...
    <div class="row">
        ...
        <?php echo get_phrase('class'); ?>&nbsp;<?php echo $row['name']; ?></option><?php endforeach;
            ...
            <div class="col-sm-12 form-group">
                ...
                <select name="student_id" class="selectpicker" data-style="form-control" data-live-search="true" id="student_holder" onchange="return onstudentchange();" data-validate="required" data-message-required ="Please select a student">                                            <option value=""><?php echo get_phrase('select_class_first'); ?></option>
                </select>
            </div>

            <div class="col-sm-12 form-group">
                <label class="control-label"><?php echo get_phrase('exams'); ?></label>
                <select name="exam_id" class="form-control" id="exam_holder" onchange="return onexamchange(this);" data-validate="required" data-message-required ="Please select a exam">
...
<script type="text/javascript">
    ...
    function onstudentchange()
    {
        ...
        $.ajax({
            url: '<?php echo base_url(); ?>index.php?school_admin/get_exams/gpa',
            success: function (response)
            {
    ...
    function onstudentchange1()
    {
        ...
        alert("here");
        $.ajax({
            url: '<?php echo base_url(); ?>index.php?school_admin/gpa_consolidated',
            type: POST,
            data: dataObj1,
            success: function (response)
            {
                jQuery('#gpa_consolidated').html(response).selectpicker('refresh');
            }
        });
    ...
    function onexamchange(exam_id)
    {
        ...
        $.ajax({
            url: '<?php echo base_url(); ?>index.php?school_admin/gpa_marks',
            type: 'POST',
            data: dataObj,
            success: function (response)
            {
                jQuery('#gpa_marksheet').html(response).selectpicker('refresh');
            }
        });
