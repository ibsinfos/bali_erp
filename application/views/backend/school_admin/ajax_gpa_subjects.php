    ...
    <div class="col-md-12">        
        <div class="white-box">
                                ...
                                <tbody><?php $count = 1; if(count($subjects)){ foreach ($subjects as $row): ?>
                                    <tr>
                                        ...
                                        <td>
                                            ...
                                            <!-- edit -->
                                            <a onclick="showAjaxModal('<?php echo base_url(); ?>index.php?modal/popup/modal_edit_gpa_subject/<?php echo $row['subject_id']; ?>');">
                                                <button type="button" class="btn btn-default btn-outline btn-circle btn-lg m-r-5 tooltip-danger" data-toggle="tooltip" data-placement="top" data-original-title="Edit Class" title="Edit Subject"><i class="fa fa-pencil-square-o"></i></button>
                                            </a>

                                            <!-- delete -->
                                            <a onclick="confirm_modal('<?php echo base_url(); ?>index.php?school_admin/gpa_subjects/delete/<?php echo $row['subject_id']; ?>');">
            ...
            <!----CREATION FORM STARTS---->
                        <section id="add">
                            <?php echo form_open(base_url() . 'index.php?school_admin/gpa_subjects/' . $class_id . '/add', array('id' => 'subject_add', 'class' => 'form-horizontal form-groups-bordered validate', 'target' => '_top')); ?>
                                <table class="table table-hover manage-u-table">
                                    ...
                                    <tbody>
                                        <?php
                                            $gpa_subject_id = array();
                                            foreach ($subjects as $row) {
                                                $gpa_subject_id[] = $row['subject_id'];
                                            }
                                            foreach ($class_subjects as $row):
                                                if (!in_array($row['subject_id'], $gpa_subject_id)) {
                                        ?>
                                        <tr>
                                            <td>
                                                <div class="form-group">
                                                    <div class="col-sm-5">
                                                        <div class="checkbox-danger">
                                                            <input type="checkbox" onclick="handleClick(this)" name="selected_subject[]" id="subject_check<?php echo $row['subject_id']; ?>" value="<?php echo $row['subject_id']; ?>">
                                                        </div>
                                                    </div>
                                                </div>
                                            </td>
                                            <td><?php echo $row['name']; ?></td>
                                            <td>
                                                <input type="text" disabled name="selected_weekly_classes[]" id="weekly<?php echo $row['subject_id']; ?>" value="" onkeypress="return isNumber(event)" required />
                                            </td>
                                            <td>
                                                <input type="text" disabled name="selected_credit_hours[]" id="credit<?php echo $row['subject_id']; ?>" value="" onkeypress="return isNumber(event)" required />
                                            </td>
                                            <td>
                                                <input type="checkbox" disabled name="selected_no_exam[]" id="no<?php echo $row['subject_id']; ?>" value="<?php echo $row['subject_id']; ?>">    
