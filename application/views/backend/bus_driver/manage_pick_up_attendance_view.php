<div class="row bg-title">
    <div class="col-lg-3 col-md-4 col-sm-4 col-xs-12">
        <h4 class="page-title"><?php echo get_phrase('manage_pick-up_attendance'); ?> </h4></div>
...
<div class="col-md-12 white-box">
    <?php echo form_open(base_url('index.php?bus_driver/manage_pick_up_attendance')); ?>
...
<div class="col-md-12 white-box" data-step="9" data-intro="Shows the student details!!" data-position='top'>
    <div class="text-center">
        ...
        <h3><?php echo get_phrase('pick_up_on').' '.date('d M Y',strtotime($date));?></h3>
    </div>
        ...
        <table id="dt_table" class="display nowrap" cellspacing="0" width="100%">
            <thead>
                <tr>
                    ...
                    <th class="text-center"><div><?php echo get_phrase('pick_up_in');?></div></th>              
                    <th class="text-center"><div><?php echo get_phrase('pick_up_out');?></div></th>
                    <!-- <th><div><?php //echo get_phrase('pick_up_time_in'); ?></div></th>
                    <th><div><?php //echo get_phrase('pick_up_time_out'); ?></div></th>  -->        
                </tr>
            </thead>
            <tbody>
                <?php $count = 1;
                    ...
                    <tr>
                        ...
                        <td class="text-center">
                            ...
                            data-size="small" <?php echo $row->pick_up_in?'checked':''?> <?php echo $row->pick_up_in?'disabled':''?>>
                        </td>
                        <td class="text-center">
                            <input type="checkbox" value="1" name="out-switch" class="js-switch out-<?php echo $row->student_id?>" 
                            data-size="small" <?php echo $row->pick_up_out?'checked':''?> <?php echo $row->pick_up_out?'disabled':''?>>
                        </td>
                        <!-- <td class="time-in"><?php //echo $row->pick_up_in?date('H:i A',strtotime($row->pick_up_in_time)):'N/A';?></td>
                        <td class="time-out"><?php //echo $row->pick_up_out?date('H:i A',strtotime($row->pick_up_out_time)):'N/A';?></td> -->
                    </tr>
                <?php endforeach; ?>
            </tbody>
        </table>
...
$(document).ready(function () {
    ...
    $('.rfid-attn').bind('change', function(e) {
        ...
        AtRec = [];
        exTable = $('#dt_table').DataTable();//.row(0).data()
        ...
        if (RFID !=false && AtRec.data('stu-id')!=undefined){//AtRec.length>0){
            ...
            $.ajax({
                type:'POST',
                url:'<?php echo base_url('index.php?ajax_controller/update_student_bus_pick_up_attendance')?>',
                ...
                success:function(res){
                    ...
                    if(res.status=='success'){
                        tr = $($('#dt_table').dataTable().fnGetNodes()[AtRec.rowIndex]);
                        tr.find('select').val(1);

                        if(res.type=='in'){
                            //changeSwitch('.in-'+AtRec.data('stu-id'),true);
                            ...
                            //changeSwitch('.out-'+AtRec.data('stu-id'),true);
