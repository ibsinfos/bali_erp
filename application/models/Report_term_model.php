...
class Report_term_model extends CI_Model {

    private $_table = "report_term";
    ...
    public function get_list() {
        ...
        $detail = $this->db->select('*')->from($this->_table)->get()->result_array();
        return $detail;
    }

    public function save_term($dataArray) {
        $school_id = '';
        if(($this->session->userdata('school_id'))) {
            $school_id = $this->session->userdata('school_id');
            if($school_id > 0){
    ...
    function get_term_data($term_id) {
        ...
        $detail = $this->db->select('*')->from($this->_table)->where('term_id', $term_id)->get()->result_array();
        return $detail;
    }
    function update_term($dataArray, $conditionArray){
