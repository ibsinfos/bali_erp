...
class Medical_camp_model extends CI_Model {

    private $_table = "medical_comp";
    private $_table_doctor = "doctors";
    ...
    function get_data_by_id($id){
     ...
     return $this->db->get_where($this->_table, array('medical_camp_id' => $id))->row();
         }
         
    function update_camp($dataArray, $condition){
...
//        echo $this->db->last_query(); die;
    ...
    function get_camp_array($dataArray = "") {
        $school_id = '';
        if(($this->session->userdata('school_id'))) {
            $school_id = $this->session->userdata('school_id');
            if($school_id > 0){
