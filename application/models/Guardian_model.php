...
if (!defined('BASEPATH'))
    ...
    function insert_guardian($dataArray){
        $school_id = '';
        if(($this->session->userdata('school_id'))) {
            $school_id = $this->session->userdata('school_id');
            if($school_id > 0){
        ...
        $sql = $this->db->insert($this->_table, $dataArray);
        $guardian_id = $this->db->insert_id($sql);            
        return $guardian_id;
    }

    function update_guardian($dataArray, $condition){
