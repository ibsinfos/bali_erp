...
class Transfer_certificate_model extends CI_Model {
    ...
    private $_table_merit = "merit_certificate";
    ...
    public function get_tc_no($student_id = '') {
        ...
        $tc_id = $this->db->select('tc_id')->from('transfer_certificate')->where('student_id',$student_id)->get()->row();    
        if(empty($tc_id)){
            $tc_id1 = $this->db->select('tc_id')->from('transfer_certificate')->order_by('id', 'desc')->get()->row();
            return $tc_id1;
        }else{
            return $tc_id;         
        }
    ...
    public function save_tc_no($dataArr){
        $school_id = '';
        if(($this->session->userdata('school_id'))) {
            $school_id = $this->session->userdata('school_id');
            if($school_id > 0){
    ...
    function save_merit_cerificate($dataArr) {
        $school_id = '';
        if(($this->session->userdata('school_id'))) {
            $school_id = $this->session->userdata('school_id');
            if($school_id > 0){
        ...
        $this->db->insert($this->_table_merit, $dataArr);
        return $this->db->insert_id();
    }
