...
//private $_table_enroll="enroll";
    ...
    public function getstudents_by_gender() {
        $school_id = '';
        if(($this->session->userdata('school_id'))) {
            $school_id = $this->session->userdata('school_id');
            if($school_id > 0){
        ...
        $this->db->select('schools.name,c.class_id, c.name class_name, sec.name section_name, count(case when sex="Male" then 1 end) as male_count, count(case when sex="Female" then 1 end) as female_count');
        ...
        $this->db->join('schools', 'c.school_id = schools.school_id');
    ...
    public function get_all_sections() {
        ...
        $this->db->select('c.class_id, c.name class_name, sec.name section_name');
        $this->db->from('class c');
        $this->db->join('section sec', 'sec.class_id = c.class_id');
...
//        echo $this->db->last_query(); die;
    ...
    public function get_student4_fi($SearchText) {
        ...
        $this->db->select('stu.name stu_fname, stu.mname stu_mname, stu.lname stu_lname, stu.birthday, stu.sex, stu.phone, stu.email, stu.caste_category, e.enroll_id, e.enroll_code, e.roll, c.name class_name, s.name section_name');
