...
class Dormitory_model extends CI_Model {
    ...
    private $_table = "dormitory";

    function __construct() {
        parent::__construct();
    }

    public function get_Student_dormatory($student_id)
    {
        $school_id = '';
        ...
        $this->db->select('d.dormitory_id, d.name dormitory_name, d.hostel_type'); 
        $this->db->from('dormitory d'); 
        $this->db->join('student s','s.dormitory_id = d.dormitory_id');
    ...
    public function get_dormitory_array($condition=array()) {
        $school_id = '';
        if(($this->session->userdata('school_id'))) {
            $school_id = $this->session->userdata('school_id');
            if($school_id > 0){
    ...
    public function get_name($param)
    {
        ...
        return $this->db->from('dormitory')->like('name',trim($param))->get()->result();
    }
    ...
    public function update($dormitory_id,$dataArray){
        $this->db->where(array('dormitory_id'=>$dormitory_id));
        $this->db->update($this->_table, $dataArray);
        return true;
    }
    public function delete($data) {
        $this->db->where('dormitory_id',$data);
