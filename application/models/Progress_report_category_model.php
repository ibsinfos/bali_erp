...
class Progress_report_category_model extends CI_Model {
    private $_table="progress_report_category";
    ...
    function get_details_by_heading($id){
        $school_id = '';
        if(($this->session->userdata('school_id'))) {
            $school_id = $this->session->userdata('school_id');
            if($school_id > 0){
                $this->db->where('prc.school_id',$school_id);
            } 
        }
        $this->db->select('*')->from($this->_table." AS prc");
        $rs=$this->db->where('prc.heading_id',$id)->get()->result_array();
        return $rs;
    }
}
