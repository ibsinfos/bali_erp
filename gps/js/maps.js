jQuery(document).ready(function($) {
    ...
    function turnOnAutoRefresh() {
        showMessage('Auto Refresh On (1 sec)'); 
    ...
    function restartInterval() {
        ...
        if (viewingAllRoutes) {
            intervalID = setInterval(getAllRoutesForMap, 5 * 1000); // one minute 
        } else {
    ...
    function update_buslive_tracking() { 
            if($('#routeSelect option:selected').val() == '0') {
                var bus_number          =   '0';
            } else {
                var bus_number          =   $('#routeSelect option:selected').text();
                bus_number              =   bus_number.split(/ +/);
            ...
            $.ajax({
                async       :   false,
                type        :   'POST',
                url         :   base_url+'madaan/index.php?Ajax_controller/live_trackupdate/'+bus_number,
